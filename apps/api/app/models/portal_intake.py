...
class PortalIntakeSubmission(Base):
    ...
    status: Mapped[PortalIntakeStatus] = mapped_column(
        SAEnum(PortalIntakeStatus), default=PortalIntakeStatus.received, nullable=False, index=True
    )
    source: Mapped[str] = mapped_column(String(100), default="portal", nullable=False)
    request_type: Mapped[str] = mapped_column(
        String(100), default="intake_form", nullable=False, index=True
    )
