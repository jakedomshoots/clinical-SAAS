...
@router.post("/{patient_id}/documents/upload", response_model=PatientDocumentUploadPrepareOut)
async def prepare_patient_document_upload(
    patient_id: str,
...
async def process_patient_document(
    ...
    result = await patient_document_service.process_patient_document(
        db,
        current_user,
        patient_id,
        document_id,
    ...
    if not result:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Document not found")
    return PatientDocumentProcessOut(**result)
