...
def _msg_to_dict(m: Message, sender_name: str | None = None, recipient_name: str | None = None) -> dict:
    return {
        "id": m.id,
...
async def list_threads(db: AsyncSession, user_id: str) -> tuple[list[dict], int]:
    ...
    for msg, unread_count in rows:
        ...
        threads.append({
            ...
            "last_message": _msg_to_dict(msg, user_map.get(msg.sender_id), user_map.get(msg.recipient_id)),
...
async def list_messages(db: AsyncSession, thread_id: str) -> list[dict]:
    ...
    return [_msg_to_dict(m, user_map.get(m.sender_id), user_map.get(m.recipient_id)) for m in messages]
...
async def get_message(db: AsyncSession, msg_id: str) -> dict | None:
    ...
    return _msg_to_dict(m, sender.display_name if sender else None, recipient.display_name if recipient else None)
