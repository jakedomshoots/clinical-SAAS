...
async def list_tasks(
    ...
    if patient_ids:
        p = await db.execute(select(Patient.id, Patient.first_name, Patient.last_name).where(Patient.id.in_(patient_ids)))
...
async def update_task(db: AsyncSession, user: User, task_id: str, data: dict) -> dict | None:
    ...
    await log_event(db, event_type, "task", task.id, actor_id=user.id, payload={"title": task.title, "status": task.status.value})
    return await get_task(db, task.id)
...
def _make_task_dict(t: Task) -> dict:
    return {
        ...
        "assigned_to_id": t.assigned_to_id, "assigned_to_name": None,
        "patient_id": t.patient_id, "patient_name": None,
