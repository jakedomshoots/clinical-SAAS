...
async def list_faxes(
    ...
    direction: str | None = None, status: str | None = None, patient_id: str | None = None,
) -> tuple[list[dict], int]:
    query = select(Fax)
...
async def send_fax(db: AsyncSession, user: User, to_number: str, patient_id: str | None = None, file_url: str | None = None) -> dict:
    fax = Fax(
        direction=FaxDirection.outbound, status=FaxStatus.pending,
        from_number="", to_number=to_number, patient_id=patient_id, file_url=file_url, pages=1,
    )
    db.add(fax)
    await db.commit()
...
def _make_fax_dict(f: Fax) -> dict:
    return {
        "id": f.id, "direction": f.direction.value, "status": f.status.value,
        "from_number": f.from_number, "to_number": f.to_number, "pages": f.pages,
        "file_url": f.file_url, "patient_id": f.patient_id, "patient_name": None,
        "matched_by": f.matched_by, "ocr_text": f.ocr_text,
