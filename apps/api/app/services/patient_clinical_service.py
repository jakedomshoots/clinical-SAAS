...
async def create_medication(
    db: AsyncSession, user: User, patient_id: str, data: dict
) -> dict | None:
    if not await _patient_exists(db, user, patient_id):
        return None
    ...
    await log_event(
        ...
        med.id,
        actor_id=user.id,
        payload={"patient_id": patient_id, "name": med.name},
    )
    return PatientMedicationOut.model_validate(med).model_dump()


async def update_medication(
    db: AsyncSession, user: User, patient_id: str, medication_id: str, data: dict
) -> dict | None:
    med = (
        await db.execute(
    ...
    for field, value in data.items():
        setattr(
            med,
            field,
            MedicationStatus(value) if field == "status" and value is not None else value,
        )
    await db.commit()
    await db.refresh(med)
    await log_event(
        db,
        "patient_medication.updated",
        "patient_medication",
        med.id,
        actor_id=user.id,
    ...
    if user_ids:
        users = await db.execute(
            select(User.id, User.display_name).where(
                User.id.in_(user_ids), User.organization_id == user.organization_id
            )
        )
...
async def create_care_plan_item(
    db: AsyncSession, user: User, patient_id: str, data: dict
) -> dict | None:
    if not await _patient_exists(db, user, patient_id):
        return None
    ...
    await log_event(
        db,
        "patient_care_plan.created",
        "patient_care_plan",
        item.id,
        actor_id=user.id,
        payload={"patient_id": patient_id, "owner_role": item.owner_role},
    )
    return PatientCarePlanItemOut.model_validate(item).model_dump()


async def update_care_plan_item(
    db: AsyncSession, user: User, patient_id: str, item_id: str, data: dict
) -> dict | None:
    item = (
        await db.execute(
    ...
    for field, value in data.items():
        setattr(
            item, field, CarePlanStatus(value) if field == "status" and value is not None else value
        )
    await db.commit()
    await db.refresh(item)
    await log_event(
        db,
        "patient_care_plan.updated",
        "patient_care_plan",
        item.id,
        actor_id=user.id,
        payload={"patient_id": patient_id, "updated_fields": list(data.keys())},
    ...
    rows = (
        (
            await db.execute(
                ...
                .order_by(
                    PatientLabResult.collected_at.desc().nulls_last(),
...
async def create_lab(db: AsyncSession, user: User, patient_id: str, data: dict) -> dict | None:
    ...
    await log_event(
        ...
        lab.id,
        actor_id=user.id,
        payload={"patient_id": patient_id, "panel": lab.panel, "flag": lab.flag},
    )
    return PatientLabResultOut.model_validate(lab).model_dump()


async def update_lab(
    db: AsyncSession, user: User, patient_id: str, lab_id: str, data: dict
) -> dict | None:
    lab = (
        await db.execute(
    ...
    for field, value in data.items():
        setattr(
            lab, field, LabResultStatus(value) if field == "status" and value is not None else value
        )
    await db.commit()
    await db.refresh(lab)
    await log_event(
...
async def create_encounter(
    ...
    await log_event(
        db,
        "patient_encounter.created",
        "patient_encounter",
        encounter.id,
        actor_id=user.id,
        payload={"patient_id": patient_id, "status": encounter.status.value},
    )
    return (await list_encounters(db, user, patient_id))[0][0]


async def update_encounter(
    db: AsyncSession, user: User, patient_id: str, encounter_id: str, data: dict
) -> dict | None:
    encounter = (
        await db.execute(
    ...
    if "provider_id" in data and data["provider_id"]:
        exists = (
            await db.execute(
                select(User.id).where(
                    User.id == data["provider_id"], User.organization_id == user.organization_id
                )
            )
        ).scalar_one_or_none()
        if not exists:
    ...
    for field, value in data.items():
        setattr(
            encounter,
            field,
            EncounterStatus(value) if field == "status" and value is not None else value,
        )
    ...
    return (
        next((item for item in listed[0] if item["id"] == encounter_id), None) if listed else None
    )
