...
DRCHRONO_MIGRATION_PACKET_EVENT_TYPE = "operations.drchrono_migration_packet"
SCOPE_ACCEPTANCE_PACKET_EVENT_TYPE = "operations.scope_acceptance_packet"
...
async def scope_acceptance_packet(db: AsyncSession, user: User) -> dict:
    packet_id = str(uuid4())
    parity = _drchrono_parity_matrix()
    outage_targets = _outage_reduction_targets()
    rollback = _rollback_scorecard()
    gaps = [item for item in parity if item["gaps"]]
    return {
        "packet_id": packet_id,
        "generated_at": datetime.now(UTC).isoformat(),
        "generated_by": user.display_name,
        "status": "blocked" if gaps else "accepted",
        "parity_matrix": parity,
        "outage_reduction_targets": outage_targets,
        "rollback_scorecard": rollback,
        "gaps": gaps,
        "gap_count": len(gaps),
...
async def scope_acceptance_packet_csv(packet: dict) -> str:
    rows = []
    rows.append("Scope Acceptance Packet")
    rows.append(f"Generated at,{packet['generated_at']}")
    rows.append(f"Generated by,{packet['generated_by']}")
    rows.append(f"Status,{packet['status']}")
    rows.append(f"Gap count,{packet['gap_count']}")
    rows.append("")
    ...
    for item in packet["outage_reduction_targets"]:
        rows.append(f"{item['target']},{item['goal']},{item['fallback']},{item['owner']}")
    rows.append("")
    rows.append("Rollback Scorecard")
    rows.append("Item,Required,Pass")
    for item in packet["rollback_scorecard"]["criteria"]:
        rows.append(f"{item['item']},{item['required']},{item['pass']}")
    return "\n".join(rows) + "\n"


def _drchrono_dry_run_analysis() -> dict:
    return {
...
async def drchrono_migration_packet(db: AsyncSession, user: User) -> dict:
    packet_id = str(uuid4())
    dry_run = _drchrono_dry_run_analysis()
    import_batches = await list_drchrono_import_batches(db, user)
    return {
        "packet_id": packet_id,
        "generated_at": datetime.now(UTC).isoformat(),
        "generated_by": user.display_name,
        "status": "blocked" if not dry_run["clinic_sign_off"] else "ready",
        "dry_run_analysis": dry_run,
        "import_batches": import_batches,
        "write_import_blocked": True,
        "write_import_block_reason": (
            "Final write import is blocked until: (1) production migration approval, "
            "(2) final DrChrono export reconciliation, and (3) a write-capable importer exists."
        ),
        "export_filename": "concierge-os-drchrono-migration-packet.json",
    }


async def drchrono_migration_dry_run(db: AsyncSession, user: User, data: dict) -> dict:
    dry_run_id = str(uuid4())
    analysis = _drchrono_dry_run_analysis()
    analysis["total_rows_analyzed"] = data.get("total_rows", 0)
    analysis["create_count"] = data.get("create_count", 0)
    analysis["update_count"] = data.get("update_count", 0)
    analysis["skip_count"] = data.get("skip_count", 0)
    analysis["duplicate_count"] = data.get("duplicate_count", 0)
    analysis["missing_dependency_count"] = data.get("missing_dependency_count", 0)
    analysis["needs_review_count"] = data.get("needs_review_count", 0)
    for section in data.get("sections", {}):
        if section in analysis["sections"]:
            analysis["sections"][section].update(data["sections"][section])
    event = await log_event(
        db,
        DRCHRONO_MIGRATION_PACKET_EVENT_TYPE,
        "operations",
        dry_run_id,
        actor_id=user.id,
        payload={
    ...
    return {
        ...
        "analysis": analysis,
        "event_id": event.id,
        "generated_at": event.created_at.isoformat(),
    }


async def create_drchrono_import_batch(db: AsyncSession, user: User, data: dict) -> dict:
    batch_id = str(uuid4())
    batch = {
        "batch_id": batch_id,
        "status": "planned",
        "mode": "no_write",
        "section": data.get("section", "patients"),
    ...
    event = await log_event(
        ...
        batch_id,
        actor_id=user.id,
        payload={"batch": batch, "event_subtype": "import_batch_created"},
    )
    return {**batch, "event_id": event.id}
...
async def list_drchrono_import_batches(db: AsyncSession, user: User) -> list[dict]:
    ...
    for event in rows:
        payload = event.payload or {}
        if payload.get("event_subtype") == "import_batch_created" and payload.get("batch"):
            batches.append(payload["batch"])
    return batches


async def drchrono_migration_packet_csv(packet: dict) -> str:
    rows = []
    rows.append("DrChrono Migration Packet")
    rows.append(f"Packet ID,{packet['packet_id']}")
    rows.append(f"Generated at,{packet['generated_at']}")
    rows.append(f"Generated by,{packet['generated_by']}")
    rows.append(f"Status,{packet['status']}")
    rows.append(f"Write import blocked,{packet['write_import_blocked']}")
    rows.append(f"Block reason,{neutralize_csv_formula(packet['write_import_block_reason'])}")
    rows.append("")
    rows.append("Dry Run Analysis")
    dry = packet["dry_run_analysis"]
    rows.append(f"Total rows analyzed,{dry['total_rows_analyzed']}")
    rows.append(f"Create count,{dry['create_count']}")
    rows.append(f"Update count,{dry['update_count']}")
    rows.append(f"Skip count,{dry['skip_count']}")
    rows.append(f"Duplicate count,{dry['duplicate_count']}")
    rows.append(f"Missing dependency count,{dry['missing_dependency_count']}")
    rows.append(f"Needs review count,{dry['needs_review_count']}")
    rows.append(f"Sample chart reviewed,{dry['sample_chart_reviewed']}")
    rows.append(f"Clinic sign off,{dry['clinic_sign_off']}")
    rows.append("")
    rows.append("Import Batches")
    rows.append("Batch ID,Section,Mode,Create,Update,Skip,Error,Status")
    for batch in packet.get("import_batches", []):
        rows.append(
            f"{batch['batch_id']},{batch['section']},{batch['mode']},"
            f"{batch['create_count']},{batch['update_count']},{batch['skip_count']},"
            f"{batch['error_count']},{batch['status']}"
        )
    return "\n".join(rows) + "\n"
