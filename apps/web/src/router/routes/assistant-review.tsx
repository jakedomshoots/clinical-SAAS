...
function AssistantReviewPage() {
  ...
  const statusFor = (event: AuditEvent) =>
    (event.payload as { status?: string })?.status ?? 'Confirmed';
    ...
    <div className="space-y-5">
      <header>
        ...
        <p className="mt-2 max-w-3xl text-small text-ink-muted">
          Every Assistant-assisted action should show status, risk, confirmation history, and audit
          payload before staff trusts it.
        </p>
      </header>
      <section className="grid gap-3 md:grid-cols-3">
        {[
          [
            'Pending review',
            rows.filter((row) => statusFor(row).toLowerCase().includes('pending')).length,
          ],
          [
            'Confirmed actions',
            rows.filter((row) => statusFor(row).toLowerCase().includes('confirmed')).length,
          ],
      ...
      <section className="flex flex-wrap gap-2 rounded-md border border-border bg-canvas-raised p-3">
        ...
        <input
          ...
          placeholder="Patient, action, payload"
          className="min-w-64 bg-canvas border border-border rounded-sm px-3 py-2 text-sm text-ink"
        />
      </section>
      {isLoading ? (
        <LoadingState label="Loading assistant actions" />
      ) : (
        <section className="overflow-hidden rounded-md border border-border bg-canvas-raised">
          <div className="divide-y divide-border">
            {rows.map((event) => (
              <div
                key={event.id}
                className="grid gap-3 px-4 py-3 md:grid-cols-[12rem_1fr_14rem] hover:bg-canvas-sunk/50"
              >
                ...
                <div>
                  <div className="mb-2 flex flex-wrap gap-2 text-xs">
                    <span className="rounded-pill bg-canvas-sunk text-ink-muted px-2 py-0.5 text-micro font-medium">
                      Status: {humanizeWorkflowLabel(statusFor(event))}
                    </span>
                    <span className="rounded-pill bg-warn/10 text-warn px-2 py-0.5 text-micro font-medium">
                      Risk: {riskFor(event.event_type)}
                    </span>
                  </div>
            ...
            {rows.length === 0 && (
              <div className="px-4 py-10 text-center text-sm text-ink-faint">
                No assistant-confirmed actions yet
              </div>
            )}
          </div>
        </section>
