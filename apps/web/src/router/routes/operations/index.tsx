...
import {
  ...
  type AuditEvent,
  type AuditReviewSummary,
  type BillingWorkQueue,
  type BrowserQaChecklist,
  type BrowserQaSession,
  type BrowserQaSessionList,
  type BrowserQaSessionStart,
  type BrowserQaSessionUpdate,
  type CredentialBinderSnapshot,
  type CredentialBinderSnapshotList,
  type CredentialDryRunBinder,
  type CutoverRunbook,
  type CutoverRunbookSession,
  type CutoverRunbookSessionList,
  type CutoverRunbookSessionStart,
  type CutoverRunbookSessionUpdate,
  type DocumentStorageReadiness,
  type GoLiveAttestation,
  type GoLiveAttestationCreate,
  type GoLivePacket,
  type IntegrationCapabilities,
  type IntegrationCutoverReadinessItem,
  type IntegrationCutoverReadinessPacket,
  type LaunchWorkplan,
  type LaunchWorkplanSnapshot,
  type LaunchWorkplanSnapshotList,
  type LiveUseRehearsal,
  type OperatorHealth,
  type OperationsAlertRuleList,
  type OperationsIncidentList,
  type OperationsIncidentTimeline,
  type PolicyApprovalChecklist,
  type PolicyApprovalSession,
  type PolicyApprovalSessionList,
  type PolicyApprovalSessionStart,
  type PolicyApprovalSessionUpdate,
  type ProductionConfigAudit,
  type ProductionRehearsalReport,
  type ProductionRehearsalSnapshot,
  type ProductionRehearsalSnapshotList,
  type ReadinessSnapshot,
  type ReadinessSnapshotList,
  type RehearsalAction,
  type RehearsalActionAssignmentUpdate,
  type RestoreDrillChecklist,
  type RestoreDrillSession,
  type RestoreDrillSessionList,
  type RestoreDrillSessionStart,
  type RestoreDrillSessionUpdate,
  type RoleDryRunChecklistList,
  type RoleDryRunSession,
  type RoleDryRunSessionList,
  type RoleDryRunSessionStart,
  type RoleDryRunSessionUpdate,
  type SessionPolicy,
  type StaffTrainingChecklist,
  type StaffTrainingSession,
  type StaffTrainingSessionList,
  type StaffTrainingSessionStart,
  type StaffTrainingSessionUpdate,
  type TaskOutreachSummary,
  type VendorCredentialRequestPacket,
  ...
  return (
    <span
      className={`inline-flex items-center gap-1.5 rounded-md border px-2 py-1 text-xs font-medium ${
        ok ? 'border-accent-soft bg-accent-soft text-accent' : 'border-warn/20 bg-warn/10 text-warn'
      }`}
    >
...
function OperationsPage() {
  ...
  const [cutoverAssignmentForms, setCutoverAssignmentForms] = useState<
    Record<string, AssignmentFormState>
  >({});
  const [dryRunSessionForm, setDryRunSessionForm] = useState<RoleDryRunSessionStart>({
    session_name: 'Clinic dry run',
    note: '',
  });
  const [dryRunItemForms, setDryRunItemForms] = useState<Record<string, DryRunItemFormState>>({});
  const [browserQaSessionForm, setBrowserQaSessionForm] = useState<BrowserQaSessionStart>({
    session_name: 'Browser QA run',
    browser: 'Chrome',
    note: '',
  });
  const [browserQaItemForms, setBrowserQaItemForms] = useState<
    Record<string, BrowserQaItemFormState>
  >({});
  const [staffTrainingSessionForm, setStaffTrainingSessionForm] =
    useState<StaffTrainingSessionStart>({
      session_name: 'Staff training',
      trainer_name: '',
      note: '',
    });
  const [staffTrainingItemForms, setStaffTrainingItemForms] = useState<
    Record<string, StaffTrainingItemFormState>
  >({});
  const [policyApprovalSessionForm, setPolicyApprovalSessionForm] =
    useState<PolicyApprovalSessionStart>({
      session_name: 'Policy approval',
      reviewer_name: '',
      note: '',
    });
  const [policyApprovalItemForms, setPolicyApprovalItemForms] = useState<
    Record<string, PolicyApprovalItemFormState>
  >({});
  const [restoreDrillSessionForm, setRestoreDrillSessionForm] = useState<RestoreDrillSessionStart>({
    session_name: 'Restore drill',
    owner_name: '',
    backup_reference: '',
    note: '',
  });
  const [restoreDrillItemForms, setRestoreDrillItemForms] = useState<
    Record<string, RestoreDrillItemFormState>
  >({});
  const [restoreDrillMetricsForm, setRestoreDrillMetricsForm] = useState({
    rto_minutes: '',
    rpo_minutes: '',
    note: '',
  });
  const [cutoverSessionForm, setCutoverSessionForm] = useState<CutoverRunbookSessionStart>({
  ...
  const [attestationForm, setAttestationForm] = useState<{
    decision: GoLiveAttestationCreate['decision'];
    note: string;
  }>({ decision: 'needs_changes', note: '' });
  ...
  const { data: events } = useQuery({
    ...
    queryFn: () =>
      api.get<ListResponse<IntegrationEvent>>('/integrations/events?page=1&page_size=12'),
  });
  const { data: auditEvents } = useQuery({
    queryKey: [...QUERY_KEYS.AUDIT, 'phi-access'],
    queryFn: () =>
      api.get<ListResponse<AuditEvent>>(
        '/audit?page=1&page_size=8&event_type=patient_document.accessed'
      ),
  });
  const { data: assistantEvents } = useQuery({
    ...
    queryFn: () =>
      api.get<ListResponse<AuditEvent>>(
        '/audit?page=1&page_size=8&event_type=assistant.task_created'
      ),
  });
  ...
  const { data: vendorCredentialPacket } = useQuery({
    ...
    queryFn: () =>
      api.get<VendorCredentialRequestPacket>(ROUTES.OPERATIONS_VENDOR_CREDENTIAL_REQUEST_PACKET),
  });
  const { data: adapterPacket } = useQuery({
    queryKey: [...QUERY_KEYS.READINESS, 'adapter-implementation-packet'],
    queryFn: () =>
      api.get<AdapterImplementationPacket>(ROUTES.OPERATIONS_ADAPTER_IMPLEMENTATION_PACKET),
  });
  const { data: cutoverReadinessPacket } = useQuery({
    queryKey: [...QUERY_KEYS.READINESS, 'integration-cutover-readiness-packet'],
    queryFn: () =>
      api.get<IntegrationCutoverReadinessPacket>(
        ROUTES.OPERATIONS_INTEGRATION_CUTOVER_READINESS_PACKET
      ),
  });
  const { data: credentialBinderSnapshots } = useQuery({
    queryKey: [...QUERY_KEYS.READINESS_SNAPSHOTS, 'credential-dry-run-binder'],
    queryFn: () =>
      api.get<CredentialBinderSnapshotList>(ROUTES.OPERATIONS_CREDENTIAL_DRY_RUN_BINDER_SNAPSHOTS),
  });
  ...
  const { data: rehearsalSnapshots } = useQuery({
    ...
    queryFn: () =>
      api.get<ProductionRehearsalSnapshotList>(ROUTES.OPERATIONS_PRODUCTION_REHEARSAL_SNAPSHOTS),
  });
  ...
  const rehearsalSnapshotMutation = useMutation({
    mutationFn: () =>
      api.post<ProductionRehearsalSnapshot>(ROUTES.OPERATIONS_PRODUCTION_REHEARSAL_SNAPSHOTS, {}),
    onSuccess: async () => {
      await queryClient.invalidateQueries({
        queryKey: [...QUERY_KEYS.READINESS_SNAPSHOTS, 'production-rehearsal'],
      });
  ...
  const workplanSnapshotMutation = useMutation({
    mutationFn: () =>
      api.post<LaunchWorkplanSnapshot>(ROUTES.OPERATIONS_LAUNCH_WORKPLAN_SNAPSHOTS, {}),
    onSuccess: async () => {
      await queryClient.invalidateQueries({
        queryKey: [...QUERY_KEYS.READINESS_SNAPSHOTS, 'launch-workplan'],
      });
  ...
  const credentialBinderSnapshotMutation = useMutation({
    mutationFn: () =>
      api.post<CredentialBinderSnapshot>(ROUTES.OPERATIONS_CREDENTIAL_DRY_RUN_BINDER_SNAPSHOTS, {}),
    onSuccess: async () => {
      await queryClient.invalidateQueries({
        queryKey: [...QUERY_KEYS.READINESS_SNAPSHOTS, 'credential-dry-run-binder'],
      });
  ...
  const assignmentMutation = useMutation({
    ...
    }) => api.post(ROUTES.OPERATIONS_REHEARSAL_ACTION_ASSIGNMENT(actionKey), data),
    onSuccess: async () => {
      await queryClient.invalidateQueries({
        queryKey: [...QUERY_KEYS.READINESS, 'production-rehearsal'],
      });
  ...
  const cutoverAssignmentMutation = useMutation({
    mutationFn: ({
      ...
      integration: string;
      data: RehearsalActionAssignmentUpdate;
    }) => api.post(ROUTES.OPERATIONS_INTEGRATION_CUTOVER_ASSIGNMENT(integration), data),
    onSuccess: async () => {
      await queryClient.invalidateQueries({
        queryKey: [...QUERY_KEYS.READINESS, 'integration-cutover-readiness-packet'],
      });
  ...
  const attestationMutation = useMutation({
    mutationFn: (data: GoLiveAttestationCreate) =>
      api.post<GoLiveAttestation>(ROUTES.OPERATIONS_GO_LIVE_ATTESTATIONS, data),
    onSuccess: async () => {
  ...
  const startDryRunSessionMutation = useMutation({
    mutationFn: (data: RoleDryRunSessionStart) =>
      api.post<RoleDryRunSession>(ROUTES.OPERATIONS_ROLE_DRY_RUN_SESSIONS, data),
    onSuccess: async () => {
  ...
  const updateDryRunSessionMutation = useMutation({
    mutationFn: ({ sessionId, data }: { sessionId: string; data: RoleDryRunSessionUpdate }) =>
      api.patch<RoleDryRunSession>(ROUTES.OPERATIONS_ROLE_DRY_RUN_SESSION(sessionId), data),
    onSuccess: async () => {
      await queryClient.invalidateQueries({
        queryKey: [...QUERY_KEYS.READINESS, 'role-dry-run-sessions'],
      });
  ...
  const startBrowserQaSessionMutation = useMutation({
    mutationFn: (data: BrowserQaSessionStart) =>
      api.post<BrowserQaSession>(ROUTES.OPERATIONS_BROWSER_QA_SESSIONS, data),
    onSuccess: async () => {
  ...
  const updateBrowserQaSessionMutation = useMutation({
    mutationFn: ({ sessionId, data }: { sessionId: string; data: BrowserQaSessionUpdate }) =>
      api.patch<BrowserQaSession>(ROUTES.OPERATIONS_BROWSER_QA_SESSION(sessionId), data),
    onSuccess: async () => {
      await queryClient.invalidateQueries({
        queryKey: [...QUERY_KEYS.READINESS, 'browser-qa-sessions'],
      });
  ...
  const startStaffTrainingSessionMutation = useMutation({
    mutationFn: (data: StaffTrainingSessionStart) =>
      api.post<StaffTrainingSession>(ROUTES.OPERATIONS_STAFF_TRAINING_SESSIONS, data),
    onSuccess: async () => {
  ...
  const updateStaffTrainingSessionMutation = useMutation({
    mutationFn: ({ sessionId, data }: { sessionId: string; data: StaffTrainingSessionUpdate }) =>
      api.patch<StaffTrainingSession>(ROUTES.OPERATIONS_STAFF_TRAINING_SESSION(sessionId), data),
    onSuccess: async () => {
      await queryClient.invalidateQueries({
        queryKey: [...QUERY_KEYS.READINESS, 'staff-training-sessions'],
      });
  ...
  const startPolicyApprovalSessionMutation = useMutation({
    mutationFn: (data: PolicyApprovalSessionStart) =>
      api.post<PolicyApprovalSession>(ROUTES.OPERATIONS_POLICY_APPROVAL_SESSIONS, data),
    onSuccess: async () => {
      setPolicyApprovalSessionForm({
        session_name: 'Policy approval',
        reviewer_name: '',
        note: '',
      });
  ...
  const updatePolicyApprovalSessionMutation = useMutation({
    mutationFn: ({ sessionId, data }: { sessionId: string; data: PolicyApprovalSessionUpdate }) =>
      api.patch<PolicyApprovalSession>(ROUTES.OPERATIONS_POLICY_APPROVAL_SESSION(sessionId), data),
    onSuccess: async () => {
      await queryClient.invalidateQueries({
        queryKey: [...QUERY_KEYS.READINESS, 'policy-approval-sessions'],
      });
  ...
  const startRestoreDrillSessionMutation = useMutation({
    mutationFn: (data: RestoreDrillSessionStart) =>
      api.post<RestoreDrillSession>(ROUTES.OPERATIONS_RESTORE_DRILL_SESSIONS, data),
    onSuccess: async () => {
      setRestoreDrillSessionForm({
        session_name: 'Restore drill',
        owner_name: '',
        backup_reference: '',
        note: '',
  ...
  const updateRestoreDrillSessionMutation = useMutation({
    mutationFn: ({ sessionId, data }: { sessionId: string; data: RestoreDrillSessionUpdate }) =>
      api.patch<RestoreDrillSession>(ROUTES.OPERATIONS_RESTORE_DRILL_SESSION(sessionId), data),
    onSuccess: async () => {
      await queryClient.invalidateQueries({
        queryKey: [...QUERY_KEYS.READINESS, 'restore-drill-sessions'],
      });
  ...
  const startCutoverSessionMutation = useMutation({
    mutationFn: (data: CutoverRunbookSessionStart) =>
      api.post<CutoverRunbookSession>(ROUTES.OPERATIONS_CUTOVER_RUNBOOK_SESSIONS, data),
    onSuccess: async () => {
      setCutoverSessionForm({
        session_name: 'Production cutover rehearsal',
        cutover_owner: '',
        scheduled_for: '',
        note: '',
  ...
  const updateCutoverSessionMutation = useMutation({
    mutationFn: ({ sessionId, data }: { sessionId: string; data: CutoverRunbookSessionUpdate }) =>
      api.patch<CutoverRunbookSession>(ROUTES.OPERATIONS_CUTOVER_RUNBOOK_SESSION(sessionId), data),
    onSuccess: async () => {
      await queryClient.invalidateQueries({
        queryKey: [...QUERY_KEYS.READINESS, 'cutover-runbook-sessions'],
      });
  ...
  const rehearsalExportHref = useMemo(() => {
    ...
    rehearsal.recommended_actions.forEach((action) =>
      rows.push([
        'action',
        action.key,
        action.label,
  ...
  const criticalActions = {
    ...
    staffTrainingPending:
      (staffTrainingChecklist?.total_items ?? 0) - (activeStaffTrainingSession?.signed_count ?? 0),
  ...
  const formForAction = (action: RehearsalAction): AssignmentFormState =>
    assignmentForms[action.key] ?? {
  ...
  const updateAssignmentForm = (actionKey: string, patch: Partial<AssignmentFormState>) => {
    setAssignmentForms((current) => ({
      ...current,
      [actionKey]: {
        ...(current[actionKey] ?? { owner_name: '', status: 'open', due_date: '', note: '' }),
        ...patch,
      },
    }));
  };
  ...
  const formForCutoverLane = (lane: IntegrationCutoverReadinessItem): AssignmentFormState =>
    cutoverAssignmentForms[lane.integration] ?? {
  ...
  const updateCutoverAssignmentForm = (
    integration: string,
    ...
    setCutoverAssignmentForms((current) => ({
      ...current,
      [integration]: {
        ...(current[integration] ?? { owner_name: '', status: 'open', due_date: '', note: '' }),
        ...patch,
      },
    }));
  };
  ...
  const dryRunItemKey = (sessionId: string, roleKey: string, itemKey: string) =>
    `${sessionId}:${roleKey}:${itemKey}`;
  const formForDryRunItem = (
    session: RoleDryRunSession,
    roleKey: string,
    itemKey: string
  ): DryRunItemFormState => {
  ...
  const updateDryRunItemForm = (
    sessionId: string,
    roleKey: string,
    itemKey: string,
    patch: Partial<DryRunItemFormState>
  ) => {
  ...
  const formForBrowserQaItem = (
    session: BrowserQaSession,
    itemKey: string
  ): BrowserQaItemFormState => {
    const key = browserQaItemKey(session.session_id, itemKey);
    const item = session.items.find((entry) => entry.key === itemKey);
    return (
      browserQaItemForms[key] ?? {
        qa_status: item?.qa_status ?? 'pending',
        item_note: item?.note ?? '',
      }
    );
  };
  const updateBrowserQaItemForm = (
    sessionId: string,
    itemKey: string,
    patch: Partial<BrowserQaItemFormState>
  ) => {
  ...
  const staffTrainingItemKey = (sessionId: string, roleKey: string, itemKey: string) =>
    `${sessionId}:${roleKey}:${itemKey}`;
  const formForStaffTrainingItem = (
    session: StaffTrainingSession,
    roleKey: string,
    itemKey: string
  ): StaffTrainingItemFormState => {
    const key = staffTrainingItemKey(session.session_id, roleKey, itemKey);
    const role = session.roles.find((item) => item.key === roleKey);
    const item = role?.items.find((entry) => entry.key === itemKey);
    return (
      staffTrainingItemForms[key] ?? {
        training_status: item?.training_status ?? 'pending',
        item_note: item?.note ?? '',
      }
    );
  };
  const updateStaffTrainingItemForm = (
    sessionId: string,
    roleKey: string,
    itemKey: string,
    patch: Partial<StaffTrainingItemFormState>
  ) => {
  ...
  const formForPolicyApprovalItem = (
    session: PolicyApprovalSession,
    itemKey: string
  ): PolicyApprovalItemFormState => {
    const key = policyApprovalItemKey(session.session_id, itemKey);
    const item = session.items.find((entry) => entry.key === itemKey);
    return (
      policyApprovalItemForms[key] ?? {
        approval_status: item?.approval_status ?? 'pending',
        item_note: item?.note ?? '',
      }
    );
  };
  const updatePolicyApprovalItemForm = (
    sessionId: string,
    itemKey: string,
    patch: Partial<PolicyApprovalItemFormState>
  ) => {
  ...
  const formForRestoreDrillItem = (
    session: RestoreDrillSession,
    itemKey: string
  ): RestoreDrillItemFormState => {
    const key = restoreDrillItemKey(session.session_id, itemKey);
    const item = session.items.find((entry) => entry.key === itemKey);
    return (
      restoreDrillItemForms[key] ?? {
        drill_status: item?.drill_status ?? 'pending',
        item_note: item?.note ?? '',
      }
    );
  };
  const updateRestoreDrillItemForm = (
    sessionId: string,
    itemKey: string,
    patch: Partial<RestoreDrillItemFormState>
  ) => {
  ...
  const submitRestoreDrillMetrics = (session: RestoreDrillSession) => {
    updateRestoreDrillSessionMutation.mutate({
      sessionId: session.session_id,
      data: {
        rto_minutes: restoreDrillMetricsForm.rto_minutes
          ? Number(restoreDrillMetricsForm.rto_minutes)
          : session.rto_minutes,
        rpo_minutes: restoreDrillMetricsForm.rpo_minutes
          ? Number(restoreDrillMetricsForm.rpo_minutes)
          : session.rpo_minutes,
        note: restoreDrillMetricsForm.note.trim() || session.note,
      },
    });
  };
  const cutoverStepKey = (sessionId: string, phaseKey: string, stepKey: string) =>
    `${sessionId}:${phaseKey}:${stepKey}`;
  const formForCutoverStep = (
    session: CutoverRunbookSession,
    phaseKey: string,
    stepKey: string
  ): CutoverStepFormState => {
  ...
  const updateCutoverStepForm = (
    sessionId: string,
    phaseKey: string,
    stepKey: string,
    patch: Partial<CutoverStepFormState>
  ) => {
    const key = cutoverStepKey(sessionId, phaseKey, stepKey);
    setCutoverStepForms((current) => ({
      ...current,
      [key]: {
        ...(current[key] ?? { step_status: 'pending', owner_name: '', step_note: '' }),
        ...patch,
      },
    }));
  };
  ...
  const submitCutoverRollback = (session: CutoverRunbookSession) => {
    const rollbackStatus =
      cutoverRollbackForm.rollback_status === 'not_reviewed' &&
      session.rollback_status !== 'not_reviewed'
    ...
    updateCutoverSessionMutation.mutate({
      ...
      data: {
        ...
        rollback_decision:
          cutoverRollbackForm.rollback_decision.trim() || session.rollback_decision || null,
      },
    });
  };
    ...
    <div className="space-y-5">
      <header className="flex flex-wrap items-end justify-between gap-4">
        ...
        <div className="flex items-center gap-2">
          ...
          <StatusBadge
            ok={ready?.operational_status === 'ok'}
            label={`Operational ${ready?.operational_status ?? 'checking'}`}
          />
          <button
        ...
        {TABS.map((tab) => (
          <button
            ...
            className={`px-4 py-2 rounded-md text-sm font-medium ${
              activeTab === tab.id ? 'bg-accent text-accent-on' : 'text-ink-muted hover:text-ink'
            }`}
          >
          ...
          {isSectionVisible('live-use-rehearsal', effectiveRole, viewMode) && liveUseRehearsal && (
            <PinnableSection sectionId="live-use-rehearsal" role={effectiveRole}>
              <ExpandableCard
                ...
                icon={ShieldCheck}
                status={
                  liveUseRehearsal?.status === 'ready'
                    ? 'complete'
                    : liveUseRehearsal?.status === 'blocked'
                      ? 'needs-attention'
                      : 'in-progress'
                }
                countComplete={liveUseRehearsal?.summary.evidence_ready_count ?? 0}
                countPending={
                  (liveUseRehearsal?.summary.evidence_total ?? 0) -
                  (liveUseRehearsal?.summary.evidence_ready_count ?? 0)
                }
              ...
              >
                <div className="grid gap-4 p-5 xl:grid-cols-[minmax(0,1fr)_24rem]">
                  <div className="space-y-3">
                    <div className="grid gap-4 md:grid-cols-2">
                      {[
                        [
                          'Evidence ready',
                          `${liveUseRehearsal.summary.evidence_ready_count ?? 0}/${liveUseRehearsal.summary.evidence_total ?? 0}`,
                        ],
                        [
                          'Workplan blockers',
                          String(liveUseRehearsal.summary.workplan_blockers ?? 0),
                        ],
                        [
                          'Credential blockers',
                          String(liveUseRehearsal.summary.credential_blockers ?? 0),
                        ],
                        [
                          'Unassigned work',
                          String(liveUseRehearsal.summary.workplan_unassigned ?? 0),
                        ],
                      ].map(([label, value]) => (
                        <div key={label} className="rounded-md border border-border bg-canvas p-5">
                          <div className="text-meta text-ink-muted">{label}</div>
                          <div className="font-serif text-3xl font-medium text-ink mt-2">
                            {value}
                          </div>
                        </div>
                      ))}
                    ...
                    <div className="grid gap-4 md:grid-cols-2">
                      {liveUseRehearsal.gates.map((gate) => (
                        <Link
                          key={gate.key}
                          to={gate.route}
                          className="rounded-md border border-border p-5 hover:bg-canvas-sunk transition-colors"
                        >
                          <div className="flex items-start justify-between gap-4">
                            <div className="min-w-0 flex-1">
                              ...
                              <div className="mt-1 text-small text-ink-muted line-clamp-2">
                                {gate.detail}
                              </div>
                            </div>
                            <span
                              className={`shrink-0 rounded-md border px-2 py-0.5 text-micro font-medium ${gate.status === 'ready' ? 'border-accent-soft bg-accent-soft text-accent' : gate.status === 'blocking' || gate.status === 'missing' ? 'border-danger/20 bg-danger/10 text-danger' : 'border-warn/20 bg-warn/10 text-warn-text'}`}
                            >
                              {gate.status}
                            </span>
                          </div>
                          <div className="mt-3 text-micro text-ink-faint">
                            {gate.captured_at
                              ? new Date(gate.captured_at).toLocaleString()
                  ...
                  <aside className="rounded-md border border-border">
                    <div className="border-b border-border px-6 py-4 text-meta font-medium text-ink-faint">
                      Next actions
                    </div>
                    <div className="divide-y divide-border">
                      {liveUseRehearsal.next_actions.slice(0, 6).map((action) => (
                        <Link
                          key={action.key}
                          to={action.route}
                          className="block px-6 py-4 hover:bg-canvas-sunk/50 transition-colors"
                        >
                          <div className="flex items-center justify-between gap-4">
                            ...
                            <span
                              className={`shrink-0 rounded-md border px-2 py-0.5 text-micro font-medium ${action.severity === 'blocking' ? 'border-danger/20 bg-danger/10 text-danger' : 'border-warn/20 bg-warn/10 text-warn-text'}`}
                            >
                              {action.severity}
                            </span>
                          </div>
                    ...
                    <div key={role.key} className="rounded-md border border-border bg-canvas">
                      <div className="border-b border-border px-6 py-4">
                        <div className="flex items-center justify-between gap-2">
                          ...
                          <span
                            className={`rounded-md border px-2 py-0.5 text-micro font-medium ${role.status === 'ready' ? 'border-accent-soft bg-accent-soft text-accent' : 'border-warn/20 bg-warn/10 text-warn-text'}`}
                          >
                            {role.ready_count}/{role.total}
                          </span>
                        </div>
                        ...
                        {role.items.map((item) => (
                          ...
                          >
                            <div className="flex items-center justify-between gap-2">
                              ...
                              <span
                                className={`h-2 w-2 rounded-full ${item.status === 'ready' ? 'bg-accent' : 'bg-warn'}`}
                              />
                            </div>
          ...
          {isSectionVisible('dry-run-session', effectiveRole, viewMode) && (
            <PinnableSection sectionId="dry-run-session" role={effectiveRole}>
              <ExpandableCard
                ...
                icon={Play}
                status={
                  activeDryRunSession?.status === 'completed'
                    ? 'complete'
                    : activeDryRunSession
                      ? 'in-progress'
                      : 'not-started'
                  ...
                  <div className="space-y-3">
                    <div className="rounded-md border border-border bg-canvas p-5">
                      ...
                      <button
                        type="button"
                        onClick={() =>
                          startDryRunSessionMutation.mutate({
                            session_name:
                              dryRunSessionForm.session_name?.trim() || 'Clinic dry run',
                        ...
                        <div className="flex items-center justify-between gap-2">
                          <div>
                            <div className="text-subhead font-semibold text-ink">
                              {activeDryRunSession.session_name}
                            </div>
                            <div className="mt-1 text-small text-ink-muted">
                              {activeDryRunSession.started_by ?? 'Staff'} ·{' '}
                              {new Date(activeDryRunSession.started_at).toLocaleString()}
                            </div>
                          </div>
                          <span
                            className={`rounded-md border px-2 py-1 text-small font-medium ${activeDryRunSession.status === 'completed' ? 'border-accent-soft bg-accent-soft text-accent' : 'border-warn/20 bg-warn/10 text-warn-text'}`}
                          >
                            {activeDryRunSession.status.replace('_', ' ')}
                          </span>
                        </div>
                        {activeDryRunSession.note && (
                          <div className="mt-2 text-small text-ink-muted">
                            {activeDryRunSession.note}
                          </div>
                        )}
                        <button
                          ...
                          disabled={
                            activeDryRunSession.status === 'completed' ||
                            updateDryRunSessionMutation.isPending
                          }
                    ...
                    <div className="grid gap-4 xl:grid-cols-2">
                      {activeDryRunSession.roles.map((role) => (
                        <div key={role.key} className="rounded-md border border-border">
                          ...
                          <div className="divide-y divide-border">
                            {role.items.map((item) => {
                              const itemForm = formForDryRunItem(
                                activeDryRunSession,
                                role.key,
                                item.key
                              );
                              return (
                                <div key={item.key} className="space-y-2 px-3 py-2">
                                  <div className="flex items-start justify-between gap-2">
                                    ...
                                    <span
                                      className={`rounded-md border px-2 py-0.5 text-micro font-medium ${item.dry_run_status === 'complete' ? 'border-accent-soft bg-accent-soft text-accent' : item.dry_run_status === 'blocked' ? 'border-danger/20 bg-danger/10 text-danger' : 'border-border bg-canvas text-ink-secondary'}`}
                                    >
                                      {item.dry_run_status}
                                    </span>
                                  </div>
                                  <div className="grid gap-2 sm:grid-cols-[8rem_minmax(0,1fr)_4.5rem]">
                                    <select
                                      ...
                                      onChange={(event) =>
                                        updateDryRunItemForm(
                                          activeDryRunSession.session_id,
                                          role.key,
                                          item.key,
                                          {
                                            dry_run_status: event.target
                                              .value as DryRunItemFormState['dry_run_status'],
                                          }
                                        )
                                      }
                                    ...
                                    <button
                                      type="button"
                                      onClick={() =>
                                        submitDryRunItem(activeDryRunSession, role.key, item.key)
                                      }
          ...
          {isSectionVisible('staff-training', effectiveRole, viewMode) && (
            <PinnableSection sectionId="staff-training" role={effectiveRole}>
              <ExpandableCard
                ...
                icon={CheckSquare}
                status={
                  activeStaffTrainingSession?.status === 'completed'
                    ? 'complete'
                  ...
                  <div className="space-y-3">
                    <div className="rounded-md border border-border bg-canvas p-5">
                      ...
                      <input
                        ...
                        onChange={(event) =>
                          setStaffTrainingSessionForm((current) => ({
                            ...current,
                            session_name: event.target.value,
                          }))
                        }
                      ...
                      <button
                        type="button"
                        onClick={() =>
                          startStaffTrainingSessionMutation.mutate({
                            session_name:
                              staffTrainingSessionForm.session_name?.trim() || 'Staff training',
                    ...
                    {activeStaffTrainingSession && (
                      <div className="rounded-md border border-border p-5">
                        <div className="flex items-center justify-between gap-2">
                          <div>
                            <div className="text-subhead font-semibold text-ink">
                              {activeStaffTrainingSession.session_name}
                            </div>
                            <div className="mt-1 text-small text-ink-muted">
                              {activeStaffTrainingSession.trainer_name ??
                                activeStaffTrainingSession.started_by ??
                                'Trainer'}{' '}
                              · {new Date(activeStaffTrainingSession.started_at).toLocaleString()}
                            </div>
                          </div>
                          <span
                            className={`rounded-md border px-2 py-1 text-small font-medium ${activeStaffTrainingSession.status === 'completed' ? 'border-accent-soft bg-accent-soft text-accent' : 'border-warn/20 bg-warn/10 text-warn-text'}`}
                          >
                            {activeStaffTrainingSession.status.replace('_', ' ')}
                          </span>
                        </div>
                        {activeStaffTrainingSession.note && (
                          <div className="mt-2 text-small text-ink-muted">
                            {activeStaffTrainingSession.note}
                          </div>
                        )}
                          ...
                          onClick={() =>
                            updateStaffTrainingSessionMutation.mutate({
                              ...
                              data: {
                                ...
                                note: activeStaffTrainingSession.note,
                              },
                            })
                          }
                          disabled={
                            activeStaffTrainingSession.status === 'completed' ||
                            updateStaffTrainingSessionMutation.isPending
                          }
                          className="mt-3 inline-flex w-full items-center justify-center gap-1.5 rounded-md border border-accent-soft px-3 py-2 text-small font-medium text-accent hover:bg-accent-soft disabled:opacity-50"
                        >
                          <CheckSquare className="h-3.5 w-3.5" />
                    ...
                    <div className="grid gap-4 xl:grid-cols-2">
                      {activeStaffTrainingSession.roles.map((role) => (
                        <div key={role.key} className="rounded-md border border-border">
                          ...
                          <div className="divide-y divide-border">
                            {role.items.map((item) => {
                              const itemForm = formForStaffTrainingItem(
                                activeStaffTrainingSession,
                                role.key,
                                item.key
                              );
                              return (
                                <div key={item.key} className="space-y-2 px-3 py-2">
                                  <div className="flex items-start justify-between gap-2">
                                    ...
                                    <span
                                      className={`rounded-md border px-2 py-0.5 text-micro font-medium ${item.training_status === 'signed' ? 'border-accent-soft bg-accent-soft text-accent' : item.training_status === 'reviewed' ? 'border-border bg-canvas-sunk text-ink-secondary' : 'border-border bg-canvas text-ink-secondary'}`}
                                    >
                                  ...
                                  <div className="grid gap-2 sm:grid-cols-[8rem_minmax(0,1fr)_4.5rem]">
                                    <select
                                      ...
                                      onChange={(event) =>
                                        updateStaffTrainingItemForm(
                                          activeStaffTrainingSession.session_id,
                                          role.key,
                                          item.key,
                                          {
                                            training_status: event.target
                                              .value as StaffTrainingItemFormState['training_status'],
                                          }
                                        )
                                      }
                                      className="rounded-md border border-border px-2 py-1.5 text-small focus:border-accent focus:outline-none"
                          ...
                          <div className="divide-y divide-border">
                            {role.items.map((item) => (
                              <Link
                                key={item.key}
                                to={item.route}
                                className="block px-3 py-2 hover:bg-canvas-sunk/50"
                              >
      ...
      {activeTab === 'systems-data' && (
        <div className="space-y-4 p-6">
          {isSectionVisible('system-integration', effectiveRole, viewMode) && adapterPacket && (
            <PinnableSection sectionId="system-integration" role={effectiveRole}>
              <ExpandableCard
                ...
                icon={PlugZap}
                status={
                  adapterPacket?.status === 'ready'
                    ? 'complete'
                    : adapterPacket?.status === 'blocked'
                      ? 'needs-attention'
                      : 'in-progress'
                }
                ...
                <div className="grid gap-4 p-5 md:grid-cols-2 xl:grid-cols-3">
                  {adapterPacket.items.map((item) => (
                    <Link
                      ...
                      to={item.route}
                      className="rounded-md border border-border bg-canvas p-5 hover:bg-canvas-sunk"
                    >
                      <div className="flex items-start justify-between gap-2">
                        <div className="min-w-0">
                          <div className="text-body font-medium text-ink">{item.label}</div>
                          <div className="mt-1 text-small text-ink-muted">
                            {item.adapter_method_ready_count}/{item.adapter_method_total} methods ·{' '}
                            {item.workflows.length} workflows
                          </div>
                        </div>
                        <div className="flex flex-col items-end gap-1">
                          <span
                            className={`rounded-md border px-2 py-0.5 text-micro font-medium ${item.priority === 'critical' ? 'border-danger/20 bg-danger/10 text-danger' : item.priority === 'high' ? 'border-warn/20 bg-warn/10 text-warn-text' : 'border-border bg-canvas-raised text-ink-secondary'}`}
                          >
                            {item.priority}
                          </span>
                          <span className="rounded-md border border-border bg-canvas-raised px-2 py-0.5 text-micro text-ink-muted">
                      ...
                      <div className="mt-3 space-y-1">
                        {item.implementation_phases.slice(0, 3).map((phase) => (
                          <div
                            key={phase.key}
                            className="flex items-start justify-between gap-2 rounded-md border border-border bg-canvas-raised px-2 py-1.5 text-small"
                          >
                            <div>
                            ...
                            <span
                              className={`rounded-md border px-1.5 py-0.5 text-micro ${phase.status === 'ready' ? 'border-accent-soft bg-accent-soft text-accent' : phase.status === 'blocked' ? 'border-danger/20 bg-danger/10 text-danger' : 'border-warn/20 bg-warn/10 text-warn-text'}`}
                            >
                              {phase.status}
                            </span>
                          </div>
                      ...
                      <div className="mt-3 text-small text-ink-muted">
                        {item.blockers[0] || item.docs[0] || 'Adapter implementation tracked.'}
                      </div>
                    </Link>
                  ))}
                </div>
          ...
          {isSectionVisible('operator-health', effectiveRole, viewMode) && operatorHealth && (
            <PinnableSection sectionId="operator-health" role={effectiveRole}>
              <ExpandableCard
                ...
                icon={Activity}
                status={
                  operatorHealth?.status === 'healthy'
                    ? 'complete'
                    : operatorHealth?.status === 'attention'
                      ? 'in-progress'
                      : 'needs-attention'
                }
                countComplete={
                  operatorHealth?.checks.filter((c) => c.status === 'healthy').length ?? 0
                }
                countPending={
                  operatorHealth?.checks.filter(
                    (c) => c.status === 'warning' || c.status === 'attention'
                  ).length ?? 0
                }
                  ...
                  <div className="grid gap-4 md:grid-cols-2">
                    {operatorHealth.checks.map((check) => (
                      <Link
                        key={check.key}
                        to={check.route}
                        className="rounded-md border border-border bg-canvas p-5 hover:bg-canvas-sunk transition-colors"
                      >
                        <div className="flex items-start justify-between gap-4">
                          <div className="min-w-0 flex-1">
                            <div className="text-body font-medium text-ink truncate">
                          ...
                          <span
                            className={`shrink-0 rounded-md border px-2 py-0.5 text-micro font-medium ${check.status === 'healthy' ? 'border-accent-soft bg-accent-soft text-accent' : check.status === 'warning' || check.status === 'attention' ? 'border-warn/20 bg-warn/10 text-warn-text' : 'border-danger/20 bg-danger/10 text-danger'}`}
                          >
                            {check.status}
                          </span>
                        </div>
                        <div className="mt-3 flex items-center justify-between text-micro text-ink-faint">
                          ...
                          <span>
                            {check.last_seen_at
                              ? new Date(check.last_seen_at).toLocaleString()
                              : 'No evidence'}
                          </span>
                        </div>
                      </Link>
                      ...
                      {operatorHealth.recommended_actions.slice(0, 5).map((action) => (
                        ...
                        >
                          <div className="flex items-center justify-between gap-4">
                            ...
                            <span
                              className={`shrink-0 rounded-md border px-2 py-0.5 text-micro font-medium ${action.severity === 'critical' ? 'border-danger/20 bg-danger/10 text-danger' : 'border-warn/20 bg-warn/10 text-warn-text'}`}
                            >
                              {action.severity}
                            </span>
                          </div>
          ...
          {isSectionVisible('document-storage', effectiveRole, viewMode) &&
            documentStorageReadiness && (
              <PinnableSection sectionId="document-storage" role={effectiveRole}>
                <ExpandableCard
                  ...
                  icon={Server}
                  status={
                    documentStorageReadiness?.status === 'ready'
                      ? 'complete'
                      : documentStorageReadiness?.status === 'attention'
                        ? 'in-progress'
                        : 'needs-attention'
                  }
                      ...
                      {documentStorageReadiness.checks.map((check) => (
                        ...
                        >
                          <div className="flex items-start justify-between gap-4">
                            ...
                            <span
                              className={`shrink-0 rounded-md border px-2 py-0.5 text-micro font-medium ${check.status === 'clear' ? 'border-accent-soft bg-accent-soft text-accent' : check.severity === 'critical' ? 'border-danger/20 bg-danger/10 text-danger' : 'border-warn/20 bg-warn/10 text-warn-text'}`}
                            >
                    ...
                    <aside className="rounded-md border border-border">
                      ...
                      <div className="divide-y divide-border">
                        {documentStorageReadiness.recent_handoffs.slice(0, 5).map((handoff) => (
                          <div
                            key={`${handoff.document_id}:${handoff.occurred_at}`}
                            className="px-3 py-2"
                          >
                            <div className="flex items-center justify-between gap-2">
                              ...
                              <span
                                className={`rounded-md border px-2 py-0.5 text-micro font-medium ${handoff.presigned && !handoff.expired ? 'border-accent-soft bg-accent-soft text-accent' : 'border-warn/20 bg-warn/10 text-warn-text'}`}
                              >
                                {handoff.presigned ? 'signed' : 'unsigned'}
                              </span>
                            </div>
                            <div className="mt-1 text-small text-ink-muted">
                              {handoff.storage_status} ·{' '}
                              {new Date(handoff.occurred_at).toLocaleString()}
                            </div>
                            <div className="mt-1 text-micro text-ink-faint">
                              {handoff.expired ? 'Expired' : 'Active'}
                              {handoff.expires_at
                                ? ` · expires ${new Date(handoff.expires_at).toLocaleString()}`
                                : ''}
                            </div>
                          </div>
                        ))}
          ...
          {isSectionVisible('production-config', effectiveRole, viewMode) &&
            productionConfigAudit && (
              <PinnableSection sectionId="production-config" role={effectiveRole}>
                <ExpandableCard
                  ...
                  icon={LockKeyhole}
                  status={
                    productionConfigAudit?.status === 'ready'
                      ? 'complete'
                      : productionConfigAudit?.status === 'attention'
                        ? 'in-progress'
                        : 'needs-attention'
                  }
                    ...
                    {productionConfigAudit.checks.map((check) => (
                      ...
                      >
                        <div className="flex items-start justify-between gap-2">
                          ...
                          <span
                            className={`rounded-md border px-2 py-0.5 text-micro font-medium ${check.ready ? 'border-accent-soft bg-accent-soft text-accent' : check.severity === 'critical' ? 'border-danger/20 bg-danger/10 text-danger' : 'border-warn/20 bg-warn/10 text-warn-text'}`}
                          >
                        ...
                        <div className="mt-2 flex flex-wrap gap-1">
                          {check.env_vars.map((envVar) => (
                            <span
                              key={envVar}
                              className="rounded-md border border-border bg-canvas-raised px-1.5 py-0.5 text-micro font-medium text-ink-muted"
                            >
                              {envVar}
                            </span>
                          ))}
                        </div>
                        <div className="mt-2 text-micro text-ink-faint">
                          {check.docs.join(' · ')}
                        </div>
                      </div>
                    ))}
          ...
          {isSectionVisible('browser-qa', effectiveRole, viewMode) && (
            <PinnableSection sectionId="browser-qa" role={effectiveRole}>
              <ExpandableCard
                ...
                icon={Camera}
                status={
                  activeBrowserQaSession?.status === 'completed'
                    ? 'complete'
                  ...
                  <div className="space-y-3">
                    <div className="rounded-md border border-border bg-canvas p-5">
                      ...
                      <input
                        ...
                        onChange={(event) =>
                          setBrowserQaSessionForm((current) => ({
                            ...current,
                            session_name: event.target.value,
                          }))
                        }
                      ...
                      <button
                        type="button"
                        onClick={() =>
                          startBrowserQaSessionMutation.mutate({
                            session_name:
                              browserQaSessionForm.session_name?.trim() || 'Browser QA run',
                    ...
                    {activeBrowserQaSession && (
                      <div className="rounded-md border border-border p-5">
                        <div className="flex items-center justify-between gap-2">
                          <div>
                            <div className="text-subhead font-semibold text-ink">
                              {activeBrowserQaSession.session_name}
                            </div>
                            <div className="mt-1 text-small text-ink-muted">
                              {activeBrowserQaSession.browser ?? 'Browser'} ·{' '}
                              {new Date(activeBrowserQaSession.started_at).toLocaleString()}
                            </div>
                          </div>
                          <span
                            className={`rounded-md border px-2 py-1 text-small font-medium ${activeBrowserQaSession.status === 'completed' ? 'border-accent-soft bg-accent-soft text-accent' : 'border-warn/20 bg-warn/10 text-warn-text'}`}
                          >
                            {activeBrowserQaSession.status.replace('_', ' ')}
                          </span>
                        </div>
                        {activeBrowserQaSession.note && (
                          <div className="mt-2 text-small text-ink-muted">
                            {activeBrowserQaSession.note}
                          </div>
                        )}
                        <button
                          ...
                          onClick={() =>
                            updateBrowserQaSessionMutation.mutate({
                              ...
                              data: {
                                session_status: 'completed',
                                note: activeBrowserQaSession.note,
                              },
                            })
                          }
                          disabled={
                            activeBrowserQaSession.status === 'completed' ||
                            updateBrowserQaSessionMutation.isPending
                          }
                          className="mt-3 inline-flex w-full items-center justify-center gap-1.5 rounded-md border border-accent-soft px-3 py-2 text-small font-medium text-accent hover:bg-accent-soft disabled:opacity-50"
                        >
                          <CheckSquare className="h-3.5 w-3.5" />
                        ...
                        return (
                          <div key={item.key} className="rounded-md border border-border p-5">
                            <div className="flex items-start justify-between gap-2">
                              ...
                              <span
                                className={`rounded-md border px-2 py-0.5 text-micro font-medium ${item.qa_status === 'passed' ? 'border-accent-soft bg-accent-soft text-accent' : item.qa_status === 'failed' ? 'border-danger/20 bg-danger/10 text-danger' : 'border-border bg-canvas text-ink-secondary'}`}
                              >
                                {item.qa_status}
                              </span>
                            </div>
                            <div className="mt-3 grid gap-2 sm:grid-cols-[8rem_minmax(0,1fr)_4.5rem]">
                              <select
                                ...
                                onChange={(event) =>
                                  updateBrowserQaItemForm(
                                    activeBrowserQaSession.session_id,
                                    item.key,
                                    {
                                      qa_status: event.target
                                        .value as BrowserQaItemFormState['qa_status'],
                                    }
                                  )
                                }
                                className="rounded-md border border-border px-2 py-1.5 text-small focus:border-accent focus:outline-none"
                              ...
                              <button
                                type="button"
                                onClick={() =>
                                  submitBrowserQaItem(activeBrowserQaSession, item.key)
                                }
                ...
                <div className="rounded-md border border-border bg-canvas-raised p-5">
                  ...
                  <div className="mt-4 space-y-2">
                    {coreChecks.map(([key, check]) => (
                      <div
                        key={key}
                        className="flex items-center justify-between rounded-md bg-canvas px-3 py-2 text-body"
                      >
                        ...
                        <StatusBadge
                          ok={check.ok}
                          label={check.ok ? 'Ready' : (check.error ?? 'Degraded')}
                        />
                      </div>
                    ))}
                  </div>
                  ...
                  <div className="mt-4 grid gap-2 md:grid-cols-2">
                    {integrations.map(([key, check]) => (
                      <div key={key} className="rounded-md border border-border bg-canvas p-5">
                        <div className="flex items-center justify-between gap-2">
                          <span className="text-body font-medium capitalize text-ink">
                            {key.replace('_', ' ')}
                          </span>
                          <StatusBadge
                            ok={check.ok}
                            label={check.configured ? 'Configured' : 'Demo'}
                          />
                        </div>
                        <div className="mt-2 font-mono text-small text-ink-muted">
                          {check.env_var}
                        </div>
                      </div>
                    ))}
          ...
          {isSectionVisible('integration-capability', effectiveRole, viewMode) && (
            <PinnableSection sectionId="integration-capability" role={effectiveRole}>
              <ExpandableCard
                ...
                status={'complete'}
                countComplete={
                  Object.values(capabilities ?? {}).filter((c) => c.configured).length ?? 0
                }
                countPending={
                  Object.values(capabilities ?? {}).filter((c) => !c.configured).length ?? 0
                }
                    ...
                    <div key={key} className="rounded-md border border-border bg-canvas p-5">
                      <div className="flex items-center justify-between gap-2">
                        ...
                        <StatusBadge
                          ...
                          label={capability.configured ? 'Live' : 'Staged'}
                        />
                      </div>
                      <div className="mt-2 text-small text-ink-muted">
                        {capability.supports.join(', ')}
                      </div>
                    </div>
                  ))}
                </div>
      ...
      {activeTab === 'compliance-security' && (
        <div className="space-y-4 p-6">
          {isSectionVisible('credential-binder', effectiveRole, viewMode) && credentialBinder && (
            <PinnableSection sectionId="credential-binder" role={effectiveRole}>
              <ExpandableCard
                ...
                icon={ClipboardList}
                status={
                  credentialBinder?.status === 'ready'
                    ? 'complete'
                    : credentialBinder?.status === 'blocked'
                      ? 'needs-attention'
                      : 'in-progress'
                }
                countComplete={credentialBinder?.archive_ready_count ?? 0}
                countPending={
                  (credentialBinder?.total ?? 0) - (credentialBinder?.archive_ready_count ?? 0)
                }
              ...
              >
                <div className="grid gap-4 p-5 md:grid-cols-2 xl:grid-cols-3">
                    ...
                    >
                      <div className="flex items-start justify-between gap-2">
                        <div className="min-w-0">
                          <div className="text-body font-medium text-ink">{item.label}</div>
                          <div className="mt-1 text-small text-ink-muted">
                            {item.vendor_profile.vendor_name || 'Vendor pending'} ·{' '}
                            {item.vendor_profile.owner_name || 'Owner pending'}
                          </div>
                        </div>
                        <span
                          className={`rounded-md border px-2 py-0.5 text-micro font-medium ${item.binder_status === 'ready' ? 'border-accent-soft bg-accent-soft text-accent' : item.binder_status === 'blocking' ? 'border-danger/20 bg-danger/10 text-danger' : 'border-warn/20 bg-warn/10 text-warn-text'}`}
                        >
                          {item.binder_status}
                        </span>
                      </div>
                      ...
                      <div className="mt-3 text-small text-ink-muted">
                        {item.blockers[0] || item.handoff_archive.detail}
                      </div>
                      <div className="mt-2 flex flex-wrap gap-1">
                        <span className="rounded-md border border-border bg-canvas-raised px-1.5 py-0.5 text-micro text-ink-muted">
                          {item.readiness_mode}
                        </span>
                        <span className="rounded-md border border-border bg-canvas-raised px-1.5 py-0.5 text-micro text-ink-muted">
                  ...
                  {(credentialBinderSnapshots?.data ?? []).slice(0, 3).map((snapshot) => (
                    <div
                      key={snapshot.id}
                      className="rounded-md border border-border bg-canvas-raised p-5"
                    >
                      <div className="flex items-start justify-between gap-2">
                        <div>
                          <div className="text-body font-medium text-ink">
                            Saved binder snapshot
                          </div>
                          <div className="mt-1 text-small text-ink-muted">
                        ...
                        <span
                          className={`rounded-md border px-2 py-0.5 text-micro font-medium ${snapshot.status === 'ready' ? 'border-accent-soft bg-accent-soft text-accent' : snapshot.status === 'blocked' ? 'border-danger/20 bg-danger/10 text-danger' : 'border-warn/20 bg-warn/10 text-warn-text'}`}
                        >
                          {snapshot.status}
                        </span>
                      </div>
                      <div className="mt-2 text-small text-ink-muted">
                        {snapshot.blocking_count} blocking, {snapshot.warning_count} warning,{' '}
                        {snapshot.archive_ready_count}/{snapshot.total} archives ready
                      </div>
                    </div>
                  ))}
                </div>
          ...
          {isSectionVisible('vendor-credential', effectiveRole, viewMode) &&
            vendorCredentialPacket && (
              <PinnableSection sectionId="vendor-credential" role={effectiveRole}>
                <ExpandableCard
                  ...
                  icon={LockKeyhole}
                  status={
                    vendorCredentialPacket?.status === 'ready'
                      ? 'complete'
                      : vendorCredentialPacket?.status === 'blocked'
                        ? 'needs-attention'
                        : 'in-progress'
                  }
                      ...
                      {vendorCredentialPacket.items.map((item) => (
                        ...
                        >
                          <div className="flex items-start justify-between gap-2">
                            ...
                            <span
                              className={`rounded-md border px-2 py-0.5 text-micro font-medium ${item.request_status === 'ready' ? 'border-accent-soft bg-accent-soft text-accent' : item.request_status === 'blocked' ? 'border-danger/20 bg-danger/10 text-danger' : 'border-warn/20 bg-warn/10 text-warn-text'}`}
                            >
                              {item.request_status}
                            </span>
                          </div>
                          ...
                          <div className="mt-3 text-small font-medium text-ink-secondary">
                    ...
                    <aside className="rounded-md border border-border bg-canvas-raised">
                      ...
                      {vendorCredentialPacket.items.slice(0, 1).map((item) => (
                        <div key={item.integration} className="p-5">
                          ...
                          <pre className="mt-3 whitespace-pre-wrap rounded-md border border-border bg-canvas p-5 text-small leading-5 text-ink-secondary">
                            {item.request_body}
                          </pre>
                        </div>
                      ))}
          ...
          {isSectionVisible('restore-drill', effectiveRole, viewMode) && (
            <PinnableSection sectionId="restore-drill" role={effectiveRole}>
              <ExpandableCard
                ...
                icon={RotateCw}
                status={
                  activeRestoreDrillSession?.status === 'completed'
                    ? 'complete'
                  ...
                  <div className="space-y-3">
                    <div className="rounded-md border border-border bg-canvas p-5">
                      ...
                      <input
                        ...
                        onChange={(event) =>
                          setRestoreDrillSessionForm((current) => ({
                            ...current,
                            session_name: event.target.value,
                          }))
                        }
                      ...
                      <input
                        ...
                        onChange={(event) =>
                          setRestoreDrillSessionForm((current) => ({
                            ...current,
                            backup_reference: event.target.value,
                          }))
                        }
                      ...
                      <button
                        type="button"
                        onClick={() =>
                          startRestoreDrillSessionMutation.mutate({
                            session_name:
                              restoreDrillSessionForm.session_name?.trim() || 'Restore drill',
                            owner_name: restoreDrillSessionForm.owner_name?.trim() || null,
                            backup_reference:
                              restoreDrillSessionForm.backup_reference?.trim() || null,
                    ...
                    {activeRestoreDrillSession && (
                      <div className="rounded-md border border-border p-5">
                        <div className="flex items-start justify-between gap-2">
                          <div>
                            <div className="text-subhead font-semibold text-ink">
                              {activeRestoreDrillSession.session_name}
                            </div>
                            <div className="mt-1 text-small text-ink-muted">
                              {activeRestoreDrillSession.owner_name ??
                                activeRestoreDrillSession.started_by ??
                                'Drill owner'}{' '}
                              · {new Date(activeRestoreDrillSession.started_at).toLocaleString()}
                            </div>
                          </div>
                          <span
                            className={`rounded-md border px-2 py-1 text-small font-medium ${activeRestoreDrillSession.status === 'completed' ? 'border-accent-soft bg-accent-soft text-accent' : 'border-warn/20 bg-warn/10 text-warn-text'}`}
                          >
                            {activeRestoreDrillSession.status.replace('_', ' ')}
                          </span>
                        </div>
                        {activeRestoreDrillSession.backup_reference && (
                          <div className="mt-2 text-small text-ink-muted">
                            Backup {activeRestoreDrillSession.backup_reference}
                          </div>
                        )}
                        {activeRestoreDrillSession.note && (
                          <div className="mt-2 text-small text-ink-muted">
                            {activeRestoreDrillSession.note}
                          </div>
                        )}
                        <div className="mt-3 grid gap-2 sm:grid-cols-2">
                          <input
                            ...
                            min="0"
                            value={
                              restoreDrillMetricsForm.rto_minutes ||
                              (activeRestoreDrillSession.rto_minutes === null
                                ? ''
                                : String(activeRestoreDrillSession.rto_minutes))
                            }
                            onChange={(event) =>
                              setRestoreDrillMetricsForm((current) => ({
                                ...current,
                                rto_minutes: event.target.value,
                              }))
                            }
                          ...
                          <input
                            ...
                            min="0"
                            value={
                              restoreDrillMetricsForm.rpo_minutes ||
                              (activeRestoreDrillSession.rpo_minutes === null
                                ? ''
                                : String(activeRestoreDrillSession.rpo_minutes))
                            }
                            onChange={(event) =>
                              setRestoreDrillMetricsForm((current) => ({
                                ...current,
                                rpo_minutes: event.target.value,
                              }))
                            }
                        ...
                        <textarea
                          value={
                            restoreDrillMetricsForm.note || activeRestoreDrillSession.note || ''
                          }
                          onChange={(event) =>
                            setRestoreDrillMetricsForm((current) => ({
                              ...current,
                        ...
                        <div className="mt-3 grid gap-2 sm:grid-cols-3">
                          <a
                            href={ROUTES.OPERATIONS_RESTORE_DRILL_SESSION_EXPORT(
                              activeRestoreDrillSession.session_id
                            )}
                          ...
                          <button
                            ...
                            onClick={() =>
                              updateRestoreDrillSessionMutation.mutate({
                                ...
                                data: {
                                  session_status: 'completed',
                                  note:
                                    restoreDrillMetricsForm.note.trim() ||
                                    activeRestoreDrillSession.note,
                                },
                              })
                            }
                            disabled={
                              activeRestoreDrillSession.status === 'completed' ||
                              updateRestoreDrillSessionMutation.isPending
                            }
                  ...
                  {activeRestoreDrillSession ? (
                    <div className="grid gap-4 xl:grid-cols-2">
                      {activeRestoreDrillSession.items.map((item) => {
                        const itemForm = formForRestoreDrillItem(
                          activeRestoreDrillSession,
                          item.key
                        );
                        return (
                          <div key={item.key} className="rounded-md border border-border p-5">
                            <div className="flex items-start justify-between gap-2">
                              ...
                              <span
                                className={`rounded-md border px-2 py-0.5 text-micro font-medium ${item.drill_status === 'complete' ? 'border-accent-soft bg-accent-soft text-accent' : item.drill_status === 'blocked' ? 'border-danger/20 bg-danger/10 text-danger' : 'border-border bg-canvas text-ink-secondary'}`}
                              >
                                {item.drill_status}
                              </span>
                            </div>
                            <div className="mt-3 grid gap-2 sm:grid-cols-[8rem_minmax(0,1fr)_4.5rem]">
                              <select
                                ...
                                onChange={(event) =>
                                  updateRestoreDrillItemForm(
                                    activeRestoreDrillSession.session_id,
                                    item.key,
                                    {
                                      drill_status: event.target
                                        .value as RestoreDrillItemFormState['drill_status'],
                                    }
                                  )
                                }
                                className="rounded-md border border-border px-2 py-1.5 text-small focus:border-accent focus:outline-none"
                              ...
                              <button
                                type="button"
                                onClick={() =>
                                  submitRestoreDrillItem(activeRestoreDrillSession, item.key)
                                }
                                disabled={updateRestoreDrillSessionMutation.isPending}
                                className="rounded-md border border-border-strong px-2 py-1.5 text-small font-medium text-ink-secondary hover:bg-canvas-sunk disabled:opacity-60"
                              >
          ...
          {isSectionVisible('policy-approval', effectiveRole, viewMode) && (
            <PinnableSection sectionId="policy-approval" role={effectiveRole}>
              <ExpandableCard
                ...
                icon={ShieldCheck}
                status={
                  activePolicyApprovalSession?.status === 'completed'
                    ? 'complete'
              ...
              >
                <div className="grid gap-4 p-5 lg:grid-cols-[22rem_minmax(0,1fr)]">
                  <div className="space-y-3">
                    <div className="rounded-md border border-border bg-canvas p-5">
                      <div className="text-meta font-medium text-ink-faint">
                        Start policy review
                      </div>
                      <input
                        value={policyApprovalSessionForm.session_name ?? ''}
                        onChange={(event) =>
                          setPolicyApprovalSessionForm((current) => ({
                            ...current,
                            session_name: event.target.value,
                          }))
                        }
                      ...
                      <button
                        type="button"
                        onClick={() =>
                          startPolicyApprovalSessionMutation.mutate({
                            session_name:
                              policyApprovalSessionForm.session_name?.trim() || 'Policy approval',
                    ...
                    {activePolicyApprovalSession && (
                      <div className="rounded-md border border-border p-5">
                        <div className="flex items-center justify-between gap-2">
                          <div>
                            <div className="text-subhead font-semibold text-ink">
                              {activePolicyApprovalSession.session_name}
                            </div>
                            <div className="mt-1 text-small text-ink-muted">
                              {activePolicyApprovalSession.reviewer_name ??
                                activePolicyApprovalSession.started_by ??
                                'Reviewer'}{' '}
                              · {new Date(activePolicyApprovalSession.started_at).toLocaleString()}
                            </div>
                          </div>
                          <span
                            className={`rounded-md border px-2 py-1 text-small font-medium ${activePolicyApprovalSession.status === 'completed' ? 'border-accent-soft bg-accent-soft text-accent' : 'border-warn/20 bg-warn/10 text-warn-text'}`}
                          >
                            {activePolicyApprovalSession.status.replace('_', ' ')}
                          </span>
                        </div>
                        {activePolicyApprovalSession.note && (
                          <div className="mt-2 text-small text-ink-muted">
                            {activePolicyApprovalSession.note}
                          </div>
                        )}
                        <button
                          ...
                          onClick={() =>
                            updatePolicyApprovalSessionMutation.mutate({
                              ...
                              data: {
                                session_status: 'completed',
                                note: activePolicyApprovalSession.note,
                              },
                            })
                          }
                          disabled={
                            activePolicyApprovalSession.status === 'completed' ||
                            updatePolicyApprovalSessionMutation.isPending
                          }
                          className="mt-3 inline-flex w-full items-center justify-center gap-1.5 rounded-md border border-accent-soft px-3 py-2 text-small font-medium text-accent hover:bg-accent-soft disabled:opacity-50"
                        >
                          <CheckSquare className="h-3.5 w-3.5" />
                  ...
                  {activePolicyApprovalSession ? (
                    <div className="grid gap-4 xl:grid-cols-2">
                      {activePolicyApprovalSession.items.map((item) => {
                        const itemForm = formForPolicyApprovalItem(
                          activePolicyApprovalSession,
                          item.key
                        );
                        return (
                          <div key={item.key} className="rounded-md border border-border p-5">
                            <div className="flex items-start justify-between gap-2">
                              <div>
                                ...
                                <div className="mt-1 text-small text-ink-muted">{item.detail}</div>
                                <div className="mt-1 text-micro text-ink-faint">
                                  {item.category} · {item.docs.join(', ')}
                                </div>
                              </div>
                              <span
                                className={`rounded-md border px-2 py-0.5 text-micro font-medium ${item.approval_status === 'approved' ? 'border-accent-soft bg-accent-soft text-accent' : item.approval_status === 'needs_changes' ? 'border-danger/20 bg-danger/10 text-danger' : 'border-border bg-canvas text-ink-secondary'}`}
                              >
                            ...
                            <div className="mt-3 grid gap-2 sm:grid-cols-[8rem_minmax(0,1fr)_4.5rem]">
                              <select
                                ...
                                onChange={(event) =>
                                  updatePolicyApprovalItemForm(
                                    activePolicyApprovalSession.session_id,
                                    item.key,
                                    {
                                      approval_status: event.target
                                        .value as PolicyApprovalItemFormState['approval_status'],
                                    }
                                  )
                                }
                                className="rounded-md border border-border px-2 py-1.5 text-small focus:border-accent focus:outline-none"
                              ...
                              <button
                                type="button"
                                onClick={() =>
                                  submitPolicyApprovalItem(activePolicyApprovalSession, item.key)
                                }
          ...
          {isSectionVisible('production-rehearsal', effectiveRole, viewMode) && rehearsal && (
            <PinnableSection sectionId="production-rehearsal" role={effectiveRole}>
              <ExpandableCard
                ...
                countUrgent={
                  rehearsal?.recommended_actions.filter((a) => a.severity === 'blocking').length ??
                  0
                }
                defaultExpanded={false}
              >
                    ...
                    {rehearsal.gates.map((gate) => (
                      ...
                      >
                        <div className="flex items-center justify-between gap-2">
                          ...
                          <span
                            className={`rounded-md border px-2 py-1 text-small font-medium ${gate.status === 'ready' ? 'border-accent-soft bg-accent-soft text-accent' : gate.status === 'warning' ? 'border-warn/20 bg-warn/10 text-warn-text' : 'border-danger/20 bg-danger/10 text-danger'}`}
                          >
                            {gate.score}%
                          </span>
                        </div>
                            ...
                            <div className="mt-3 grid gap-2">
                              <div className="grid grid-cols-[minmax(0,1fr)_8rem] gap-2">
                                <input
                                  ...
                                  onChange={(event) =>
                                    updateAssignmentForm(action.key, {
                                      owner_name: event.target.value,
                                    })
                                  }
                                  placeholder="Owner"
                              ...
                              <div className="grid grid-cols-[8.5rem_minmax(0,1fr)] gap-2">
                                ...
                                <input
                                  value={form.note}
                                  onChange={(event) =>
                                    updateAssignmentForm(action.key, { note: event.target.value })
                                  }
                              ...
                              <div className="flex items-center justify-between gap-2">
                                <span className="truncate text-micro text-ink-faint">
                                  {action.assignment
                                    ? `Assigned ${new Date(action.assignment.assigned_at).toLocaleDateString()}`
                                    : 'No owner assigned'}
                                </span>
                                <button
                                  type="button"
                ...
                {(rehearsalSnapshots?.data ?? []).length > 0 && (
                  <div className="border-t border-border px-6 py-4">
                    <div className="mb-2 text-meta font-medium text-ink-faint">
                      Saved rehearsal evidence
                    </div>
                    <div className="grid gap-2 md:grid-cols-3">
                      {(rehearsalSnapshots?.data ?? []).slice(0, 3).map((snapshot) => (
                        ...
                        >
                          ...
                          <div className="mt-1 text-small text-ink-muted">
                            {snapshot.blocking_count} blocker(s), {snapshot.warning_count}{' '}
                            warning(s)
                          </div>
                          <div className="mt-1 text-micro text-ink-faint">
                            {new Date(snapshot.created_at).toLocaleString()}
            ...
            <PinnableSection sectionId="audit-links" role={effectiveRole}>
              ...
              >
                <div className="rounded-md border border-border bg-canvas-raised p-5">
                  ...
                  <div className="mt-1 text-small text-ink-muted">
                    Download scoped audit CSV for compliance review
                  </div>
                  <div className="mt-3 grid gap-2">
                    <input
                      placeholder="Event type"
                      value={auditExport.event_type}
                      onChange={(event) =>
                        setAuditExport({ ...auditExport, event_type: event.target.value })
                      }
                      className="rounded-md border border-border-strong px-2 py-1.5 text-small"
                    />
                    <input
                      placeholder="Entity type"
                      value={auditExport.entity_type}
                      onChange={(event) =>
                        setAuditExport({ ...auditExport, entity_type: event.target.value })
                      }
                      className="rounded-md border border-border-strong px-2 py-1.5 text-small"
                    />
                    <input
                      placeholder="Entity ID"
                      value={auditExport.entity_id}
                      onChange={(event) =>
                        setAuditExport({ ...auditExport, entity_id: event.target.value })
                      }
                      className="rounded-md border border-border-strong px-2 py-1.5 text-small"
                    />
                    <select
                      value={auditExport.limit}
                      onChange={(event) =>
                        setAuditExport({ ...auditExport, limit: event.target.value })
                      }
                      className="rounded-md border border-border-strong px-2 py-1.5 text-small"
                    >
                    ...
                    <a
                      ...
                      className="rounded-md bg-accent text-accent-on px-3 py-2 text-center text-small font-medium hover:bg-accent-hover"
                    >
                      Export CSV
                    </a>
                  </div>
                ...
                <div className="rounded-md border border-border bg-canvas-raised p-5">
                  ...
                  <div className="mt-3 text-subhead font-semibold text-ink">
                    PHI access controls
                  </div>
                  <div className="mt-1 text-small text-ink-muted">
                    Patient document access uses expiring viewer metadata
                  </div>
                </div>
                <div className="rounded-md border border-border bg-canvas-raised p-5">
                  ...
                  <div className="mt-1 text-small text-ink-muted">
                    Production readiness tracked in operations docs
                  </div>
                </div>
              </ExpandableCard>
            </PinnableSection>
          ...
          {isSectionVisible('audit-review', effectiveRole, viewMode) && auditReviewSummary && (
            <PinnableSection sectionId="audit-review" role={effectiveRole}>
              <ExpandableCard
                ...
                icon={ShieldCheck}
                status={
                  (auditReviewSummary?.sensitive_event_count ?? 0) === 0
                    ? 'complete'
                    : 'needs-attention'
                }
                countComplete={
                  (auditReviewSummary?.total_event_count ?? 0) -
                  (auditReviewSummary?.sensitive_event_count ?? 0)
                }
              ...
              >
                <div className="flex flex-wrap items-center justify-between gap-4">
                  <div>
                    ...
                    <div className="mt-1 text-small text-ink-muted">
                      {auditReviewSummary.sensitive_event_count} sensitive event
                      {auditReviewSummary.sensitive_event_count === 1 ? '' : 's'} across{' '}
                      {auditReviewSummary.total_event_count} total audit rows
                    </div>
                  </div>
                  <span
                    className={`rounded-md border px-2 py-1 text-small font-medium ${auditReviewSummary.sensitive_event_count ? 'border-warn/20 bg-warn/10 text-warn-text' : 'border-accent-soft bg-accent-soft text-accent'}`}
                  >
                    {auditReviewSummary.recommended_actions.length} action
                    {auditReviewSummary.recommended_actions.length === 1 ? '' : 's'}
                  </span>
                </div>
                <div className="mt-3 grid gap-4 md:grid-cols-3">
                  {auditReviewSummary.categories.map((category) => (
                    <div
                      key={category.key}
                      className="rounded-md border border-border bg-canvas px-3 py-3"
                    >
                      <div className="flex items-center justify-between gap-2">
                        ...
                        <span
                          className={`rounded border px-1.5 py-0.5 text-micro font-medium ${category.severity === 'critical' ? 'border-danger/20 bg-danger/10 text-danger' : category.severity === 'warning' ? 'border-warn/20 bg-warn/10 text-warn-text' : 'border-accent-soft bg-accent-soft text-accent'}`}
                        >
                          {category.severity}
                        </span>
                      </div>
                      ...
                      <div className="mt-1 text-micro text-ink-faint">
                        {category.last_event_at
                          ? new Date(category.last_event_at).toLocaleString()
                          : 'No events'}
                      </div>
                    </div>
                  ))}
                ...
                <div className="mt-3 grid gap-2 md:grid-cols-2">
                  {auditReviewSummary.recommended_actions.slice(0, 4).map((action) => (
                    <div key={action.key} className="rounded-md border border-border px-3 py-2">
                      <div className="flex items-center justify-between gap-2">
                        <div className="text-body font-medium text-ink">{action.label}</div>
                        <span
                          className={`rounded border px-1.5 py-0.5 text-micro font-medium ${action.severity === 'critical' ? 'border-danger/20 bg-danger/10 text-danger' : 'border-warn/20 bg-warn/10 text-warn-text'}`}
                        >
                          {action.severity}
                        </span>
                      </div>
                  ...
                  {auditReviewSummary.recommended_actions.length === 0 && (
                    <div className="rounded-md border border-accent-soft bg-accent-soft px-3 py-2 text-body text-accent">
                      No sensitive audit review actions are currently open.
                    </div>
                  )}
                </div>
              </ExpandableCard>
                ...
                <div className="rounded-md border border-border bg-canvas-raised p-5">
                  ...
                  <div className="mt-4 space-y-2">
                    {deployment.map(([key, check]) => (
                      <div
                        key={key}
                        className="flex items-center justify-between gap-4 rounded-md bg-canvas px-3 py-2 text-body"
                      >
                        <div>
                          <div className="capitalize text-ink-secondary">
                            {key.replaceAll('_', ' ')}
                          </div>
                          {check.path && (
                            <div className="font-mono text-micro text-ink-faint">{check.path}</div>
                          )}
                        </div>
                    ...
                    {deployment.length === 0 && (
                      <EmptyState title="Deployment checks are not available" />
                    )}
                  </div>
                </div>
                <div className="rounded-md border border-border bg-canvas-raised">
                  <div className="flex items-center justify-between border-b border-border px-6 py-4">
                    <div>
                      ...
                      <p className="text-small text-ink-muted">
                        Document viewer access reasons and timestamps
                      </p>
                    </div>
                  ...
                  <div className="divide-y divide-border">
                    {(auditEvents?.data ?? []).map((event) => (
                      <div
                        key={event.id}
                        className="grid gap-2 px-6 py-4 text-body md:grid-cols-[1fr_12rem]"
                      >
                        <div>
                          <div className="font-medium text-ink">
                            {String(event.payload?.document_title ?? event.entity_id)}
                          </div>
                          {typeof event.payload?.reason === 'string' && (
                            <div className="mt-1 text-small text-ink-secondary">
                              {event.payload.reason}
                            </div>
                          )}
                        </div>
                        <div className="text-small text-ink-muted md:text-right">
                          {new Date(event.created_at).toLocaleString()}
                        </div>
                      </div>
                    ))}
                    {(auditEvents?.data ?? []).length === 0 && (
                      <EmptyState title="No PHI access events recorded yet" />
                    )}
                  </div>
                </div>
              </ExpandableCard>
      ...
      {activeTab === 'go-live' && (
        <div className="space-y-4 p-6">
          {isSectionVisible('cutover-readiness', effectiveRole, viewMode) &&
            cutoverReadinessPacket && (
              <PinnableSection sectionId="cutover-readiness" role={effectiveRole}>
                <ExpandableCard
                  ...
                  icon={ShieldCheck}
                  status={
                    cutoverReadinessPacket?.status === 'ready'
                      ? 'complete'
                      : cutoverReadinessPacket?.status === 'blocked'
                        ? 'needs-attention'
                        : 'in-progress'
                  }
                        ...
                        >
                          <div className="flex items-start justify-between gap-2">
                            <div className="min-w-0">
                              <div className="text-body font-medium text-ink">{item.label}</div>
                              <div className="mt-1 text-small text-ink-muted">
                                {item.readiness_mode} · {item.adapter.implementation_status} adapter
                                · {item.credential_request.request_status} credential request
                              </div>
                            </div>
                            <div className="flex flex-col items-end gap-1">
                              <span
                                className={`rounded-md border px-2 py-0.5 text-micro font-medium ${item.go_no_go === 'go' ? 'border-accent-soft bg-accent-soft text-accent' : item.go_no_go === 'no_go' ? 'border-danger/20 bg-danger/10 text-danger' : 'border-warn/20 bg-warn/10 text-warn-text'}`}
                              >
                                {item.go_no_go}
                              </span>
                              <span className="rounded-md border border-border bg-canvas-raised px-2 py-0.5 text-micro text-ink-muted">
                          ...
                          <div className="mt-3 grid gap-2 sm:grid-cols-2">
                            {item.gates.slice(0, 4).map((gate) => (
                              <div
                                key={gate.key}
                                className="rounded-md border border-border bg-canvas-raised px-2 py-1.5 text-small"
                              >
                                <div className="flex items-center justify-between gap-2">
                                  <span className="font-medium text-ink">{gate.label}</span>
                                  <span
                                    className={`rounded-md border px-1.5 py-0.5 text-micro ${gate.status === 'ready' ? 'border-accent-soft bg-accent-soft text-accent' : gate.status === 'blocked' ? 'border-danger/20 bg-danger/10 text-danger' : 'border-warn/20 bg-warn/10 text-warn-text'}`}
                                  >
                                    {gate.status}
                                  </span>
                                </div>
                          ...
                          <div className="mt-3 text-small text-ink-muted">
                            {item.next_actions[0] ||
                              item.blockers[0] ||
                              'Cutover lane is ready for go/no-go review.'}
                          </div>
                          <div className="mt-3 rounded-md border border-border bg-canvas-raised p-2">
                            ...
                            <div className="grid gap-2 sm:grid-cols-[minmax(0,1fr)_8.5rem]">
                              <input
                                value={form.owner_name}
                                onChange={(event) =>
                                  updateCutoverAssignmentForm(item.integration, {
                                    owner_name: event.target.value,
                                  })
                                }
                                placeholder="Owner"
                            ...
                            <div className="mt-2 flex items-center justify-between gap-2">
                              ...
                              <button
                                ...
                                disabled={
                                  !form.owner_name.trim() || cutoverAssignmentMutation.isPending
                                }
                                className="rounded-md bg-accent text-accent-on px-2.5 py-1.5 text-small font-medium hover:bg-accent-hover disabled:opacity-60"
                              >
                                Assign
          ...
          {isSectionVisible('go-live-packet', effectiveRole, viewMode) && goLivePacket && (
            <PinnableSection sectionId="go-live-packet" role={effectiveRole}>
              <ExpandableCard
                ...
                icon={CheckCircle2}
                status={
                  goLivePacket?.go_live_ready
                    ? 'complete'
                    : goLivePacket?.blocking_count > 0
                      ? 'needs-attention'
                      : 'in-progress'
                }
                countComplete={goLivePacket?.evidence_ready_count ?? 0}
                countPending={
                  (goLivePacket?.evidence_total ?? 0) - (goLivePacket?.evidence_ready_count ?? 0)
                }
                    ...
                    {goLivePacket.evidence.map((item) => (
                      ...
                      >
                        <div className="flex items-center justify-between gap-2">
                          ...
                          <span
                            className={`rounded-md border px-2 py-1 text-small font-medium ${item.status === 'ready' ? 'border-accent-soft bg-accent-soft text-accent' : item.status === 'warning' ? 'border-warn/20 bg-warn/10 text-warn-text' : 'border-danger/20 bg-danger/10 text-danger'}`}
                          >
                            {item.status}
                          </span>
                        </div>
                    ...
                    <div className="space-y-2 border-b border-border px-3 py-3">
                      <select
                        ...
                        onChange={(event) =>
                          setAttestationForm((current) => ({
                            ...current,
                            decision: event.target.value as GoLiveAttestationCreate['decision'],
                          }))
                        }
                        className="w-full rounded-md border border-border px-2 py-1.5 text-small focus:border-accent focus:outline-none"
                      >
                        <option value="needs_changes">Needs changes</option>
                        <option value="approved" disabled={!goLivePacket.go_live_ready}>
                          Approved
                        </option>
                      ...
                      <button
                        ...
                        onClick={() =>
                          attestationMutation.mutate({
                            ...
                            note: attestationForm.note.trim() || null,
                          })
                        }
                        disabled={
                          attestationMutation.isPending ||
                          (attestationForm.decision === 'approved' && !goLivePacket.go_live_ready)
                        }
                      ...
                      {goLivePacket.latest_attestation && (
                        <div className="rounded-md border border-border bg-canvas p-2 text-small text-ink-muted">
                          <div className="font-medium text-ink">
                            {goLivePacket.latest_attestation.decision.replace('_', ' ')}
                          </div>
                          <div className="mt-1">
                            {goLivePacket.latest_attestation.reviewer_name ?? 'Reviewer'} ·{' '}
                            {new Date(goLivePacket.latest_attestation.created_at).toLocaleString()}
                          </div>
                          {goLivePacket.latest_attestation.note && (
                            <div className="mt-1">{goLivePacket.latest_attestation.note}</div>
                          )}
                          <HiddenSectionsButton
                            role={effectiveRole}
                            onUnhide={() => {}}
                    ...
                    <div className="divide-y divide-border">
                      {goLivePacket.open_workplan_items.slice(0, 4).map((item) => (
                        ...
                        >
                          ...
                          <div className="mt-1 text-micro text-ink-faint">
                            {item.assignment?.owner_name ?? 'Unassigned'} · {item.severity}
                          </div>
                        </Link>
                      ))}
          ...
          {isSectionVisible('cutover-runbook', effectiveRole, viewMode) && (
            <PinnableSection sectionId="cutover-runbook" role={effectiveRole}>
              <ExpandableCard
                ...
                icon={RefreshCw}
                status={
                  activeCutoverSession?.status === 'completed'
                    ? 'complete'
                    : activeCutoverSession?.status === 'aborted'
                      ? 'needs-attention'
                ...
                countUrgent={
                  (activeCutoverSession?.blocked_count ?? 0) +
                  (activeCutoverSession?.rollback_count ?? 0)
                }
                defaultExpanded={false}
              >
                <div className="grid gap-4 p-5 lg:grid-cols-[22rem_minmax(0,1fr)]">
                    ...
                    <div className="rounded-md border border-border bg-canvas p-5">
                      ...
                      <input
                        ...
                        onChange={(event) =>
                          setCutoverSessionForm((current) => ({
                            ...current,
                            cutover_owner: event.target.value,
                          }))
                        }
                      ...
                      <button
                        type="button"
                        onClick={() =>
                          startCutoverSessionMutation.mutate({
                            session_name:
                              cutoverSessionForm.session_name?.trim() ||
                      ...
                      <div className="space-y-3">
                        <div className="rounded-md border border-border p-5">
                          <div className="flex items-center justify-between gap-2">
                            <div>
                              <div className="text-subhead font-semibold text-ink">
                                {activeCutoverSession.session_name}
                              </div>
                              <div className="mt-1 text-small text-ink-muted">
                                {activeCutoverSession.cutover_owner ??
                                  activeCutoverSession.started_by ??
                                  'Cutover owner'}{' '}
                                · {new Date(activeCutoverSession.started_at).toLocaleString()}
                              </div>
                            </div>
                            <span
                              className={`rounded-md border px-2 py-1 text-small font-medium ${activeCutoverSession.status === 'completed' ? 'border-accent-soft bg-accent-soft text-accent' : activeCutoverSession.status === 'aborted' ? 'border-danger/20 bg-danger/10 text-danger' : 'border-warn/20 bg-warn/10 text-warn-text'}`}
                            >
                              {activeCutoverSession.status.replace('_', ' ')}
                            </span>
                          </div>
                          {activeCutoverSession.scheduled_for && (
                            <div className="mt-2 text-small text-ink-muted">
                              Scheduled{' '}
                              {new Date(activeCutoverSession.scheduled_for).toLocaleString()}
                            </div>
                          )}
                          {activeCutoverSession.note && (
                            <div className="mt-2 text-small text-ink-muted">
                              {activeCutoverSession.note}
                            </div>
                          )}
                          <div className="mt-3 grid gap-2 sm:grid-cols-3">
                            <a
                              href={ROUTES.OPERATIONS_CUTOVER_RUNBOOK_SESSION_EXPORT(
                                activeCutoverSession.session_id
                              )}
                            ...
                            <button
                              ...
                              onClick={() =>
                                updateCutoverSessionMutation.mutate({
                                  ...
                                  data: {
                                    session_status: 'completed',
                                    note: activeCutoverSession.note,
                                  },
                                })
                              }
                              disabled={
                                activeCutoverSession.status === 'completed' ||
                                updateCutoverSessionMutation.isPending
                              }
                              className="inline-flex items-center justify-center gap-1.5 rounded-md border border-accent-soft px-3 py-2 text-small font-medium text-accent hover:bg-accent-soft disabled:opacity-50"
                            >
                              <CheckSquare className="h-3.5 w-3.5" />
                        ...
                        <div className="rounded-md border border-border p-5">
                          ...
                          <div className="mt-2 grid gap-2">
                            <select
                              value={
                                cutoverRollbackForm.rollback_status === 'not_reviewed' &&
                                activeCutoverSession.rollback_status !== 'not_reviewed'
                                  ? activeCutoverSession.rollback_status
                                  : cutoverRollbackForm.rollback_status
                              }
                              onChange={(event) =>
                                setCutoverRollbackForm((current) => ({
                                  ...current,
                                  rollback_status: event.target.value as NonNullable<
                                    CutoverRunbookSessionUpdate['rollback_status']
                                  >,
                                }))
                              }
                              className="w-full rounded-md border border-border px-2 py-1.5 text-small focus:border-accent focus:outline-none"
                            ...
                            <textarea
                              value={
                                cutoverRollbackForm.rollback_decision ||
                                activeCutoverSession.rollback_decision ||
                                ''
                              }
                              onChange={(event) =>
                                setCutoverRollbackForm((current) => ({
                                  ...current,
                                  rollback_decision: event.target.value,
                                }))
                              }
                              rows={3}
                        ...
                        <div key={phase.key} className="rounded-md border border-border">
                          ...
                          <div className="divide-y divide-border">
                            {phase.steps.map((step) => {
                              const stepForm = formForCutoverStep(
                                activeCutoverSession,
                                phase.key,
                                step.key
                              );
                                ...
                                <div key={step.key} className="space-y-2 px-3 py-2">
                                  <div className="flex items-start justify-between gap-2">
                                    <div>
                                      ...
                                      <div className="mt-1 text-micro text-ink-faint">
                                        T{step.expected_minute >= 0 ? '+' : ''}
                                        {step.expected_minute} · {step.owner_role}
                                      </div>
                                    ...
                                    <span
                                      className={`rounded-md border px-2 py-0.5 text-micro font-medium ${step.step_status === 'complete' ? 'border-accent-soft bg-accent-soft text-accent' : step.step_status === 'blocked' || step.step_status === 'rollback' ? 'border-danger/20 bg-danger/10 text-danger' : 'border-border bg-canvas text-ink-secondary'}`}
                                    >
                                      {step.step_status}
                                    </span>
                                  </div>
                                  <div className="grid gap-2 sm:grid-cols-[8rem_minmax(0,1fr)]">
                                    <select
                                      ...
                                      onChange={(event) =>
                                        updateCutoverStepForm(
                                          activeCutoverSession.session_id,
                                          phase.key,
                                          step.key,
                                          {
                                            step_status: event.target
                                              .value as CutoverStepFormState['step_status'],
                                          }
                                        )
                                      }
                                      className="rounded-md border border-border px-2 py-1.5 text-small focus:border-accent focus:outline-none"
                                  ...
                                  <div className="grid gap-2 sm:grid-cols-[minmax(0,1fr)_4.5rem]">
                                    ...
                                    <button
                                      type="button"
                                      onClick={() =>
                                        submitCutoverStep(activeCutoverSession, phase.key, step.key)
                                      }
                                      disabled={updateCutoverSessionMutation.isPending}
                                      className="rounded-md border border-border-strong px-2 py-1.5 text-small font-medium text-ink-secondary hover:bg-canvas-sunk disabled:opacity-60"
                                    >
                            ...
                            {phase.steps.map((step) => (
                              <div key={step.key} className="px-3 py-2">
                                <div className="flex items-start justify-between gap-2">
                                  ...
                                  <span className="rounded-md border border-border bg-canvas-raised px-2 py-0.5 text-micro font-medium text-ink-muted">
                                    T{step.expected_minute >= 0 ? '+' : ''}
                                    {step.expected_minute}
                                  </span>
                                </div>
                                <div className="mt-1 text-micro text-ink-faint">
                                  {step.owner_role}
          ...
          {isSectionVisible('launch-workplan', effectiveRole, viewMode) && workplan && (
            <PinnableSection sectionId="launch-workplan" role={effectiveRole}>
              <ExpandableCard
                ...
                icon={ClipboardList}
                status={
                  workplan?.status === 'clear'
                    ? 'complete'
                    : workplan?.blocking_count > 0
                      ? 'needs-attention'
                      : 'in-progress'
                }
                countComplete={
              ...
              >
                <div className="divide-y divide-border">
                  {workplan.items.slice(0, 8).map((item) => (
                    <div
                      key={item.key}
                      className="grid gap-4 px-6 py-4 lg:grid-cols-[minmax(0,1fr)_11rem_8rem]"
                    >
                      <Link to={item.route} className="min-w-0 hover:text-accent">
                        <div className="flex flex-wrap items-center gap-2">
                          <span className="text-body font-medium text-ink">{item.label}</span>
                          <span
                            className={`rounded-md border px-2 py-0.5 text-micro font-medium ${item.severity === 'blocking' ? 'border-danger/20 bg-danger/10 text-danger' : 'border-warn/20 bg-warn/10 text-warn-text'}`}
                          >
                            {item.severity}
                          </span>
                          <span className="rounded-md border border-border bg-canvas px-2 py-0.5 text-micro font-medium text-ink-muted">
                            {item.category}
                          </span>
                        </div>
                        <div className="mt-1 text-small text-ink-muted">{item.detail}</div>
                      ...
                      <div className="text-small text-ink-muted">
                        <div className="font-medium text-ink-secondary">
                          {item.assignment?.owner_name ?? item.owner_role}
                        </div>
                        <div className="mt-1">
                          {item.assignment
                        ...
                        {item.assignment?.due_date && (
                          <div className="mt-1">Due {item.assignment.due_date}</div>
                        )}
                      </div>
                      <Link
                        to={item.route}
                        className="inline-flex h-9 items-center justify-center rounded-md border border-border-strong px-3 text-small font-medium text-ink-secondary hover:bg-canvas-sunk"
                      >
                        Open
                      </Link>
                    </div>
                    ...
                    <div className="grid gap-2 md:grid-cols-3">
                      {(workplanSnapshots?.data ?? []).slice(0, 3).map((snapshot) => (
                        ...
                        >
                          <div className="flex items-center justify-between gap-2">
                            <span className="text-body font-medium text-ink">
                              {snapshot.total} item(s)
                            </span>
                            <StatusBadge ok={snapshot.status === 'clear'} label={snapshot.status} />
                          </div>
                          <div className="mt-1 text-small text-ink-muted">
                            {snapshot.blocking_count} blocking, {snapshot.unassigned_count}{' '}
                            unassigned
                          </div>
                          <div className="mt-1 text-micro text-ink-faint">
                            {new Date(snapshot.created_at).toLocaleString()}
                ...
                {(snapshots?.data ?? []).slice(0, 4).map((snapshot) => (
                  ...
                  >
                    <div className="flex items-center justify-between gap-2">
                      ...
                      <StatusBadge
                        ok={snapshot.operational_status === 'ok'}
                        label={snapshot.operational_status}
                      />
                    </div>
                    <div className="mt-3 font-serif text-3xl font-medium text-ink">
                      {snapshot.launch_score}%
                    </div>
                    <div className="mt-1 text-small text-ink-muted">
                      {snapshot.incident_count} incidents · {snapshot.critical_count} critical
                    </div>
                    <div className="mt-2 text-micro text-ink-faint">
                      {new Date(snapshot.created_at).toLocaleString()}
                    </div>
              ...
              >
                <div className="grid gap-0 divide-y divide-border">
                  {(incidents?.data ?? []).slice(0, 8).map((incident) => (
                    <div
                      key={incident.key}
                      className="grid gap-4 px-6 py-4 text-body md:grid-cols-[12rem_minmax(0,1fr)_10rem]"
                    >
                      <div>
                        <div className="font-medium text-ink">{incident.title}</div>
                        <div className="mt-1 text-small capitalize text-ink-muted">
                          {incident.source.replace('_', ' ')} · {incident.status.replace('_', ' ')}
                        </div>
                      </div>
                      <div>
                      ...
                      <div className="flex items-start justify-between gap-2 md:justify-end">
                        <span
                          className={`rounded-md border px-2 py-1 text-small font-medium ${incident.severity === 'critical' ? 'border-danger/20 bg-danger/10 text-danger' : 'border-warn/20 bg-warn/10 text-warn-text'}`}
                        >
                          {incident.count} {incident.severity}
                        </span>
                        <span className="rounded-md border border-border bg-canvas px-2 py-1 text-small capitalize text-ink-secondary">
                          {incident.owner_role}
                        </span>
                      </div>
                    </div>
          ...
          {isSectionVisible('incident-timeline', effectiveRole, viewMode) && (
            <PinnableSection sectionId="incident-timeline" role={effectiveRole}>
              <ExpandableCard
                ...
                icon={Activity}
                status={
                  (incidentTimeline?.critical_count ?? 0) === 0 ? 'complete' : 'needs-attention'
                }
                countComplete={
                  (incidentTimeline?.data.length ?? 0) - (incidentTimeline?.critical_count ?? 0)
                }
              ...
              >
                <div className="rounded-md border border-border bg-canvas-raised">
                  <div className="flex flex-wrap items-center justify-between gap-4 border-b border-border px-6 py-4">
                    <div>
                      ...
                      <p className="text-small text-ink-muted">
                        {incidentTimeline?.total ?? 0} recent signal(s)
                      </p>
                    </div>
                    <div className="flex items-center gap-2">
                      <StatusBadge
                        ok={(incidentTimeline?.critical_count ?? 0) === 0}
                        label={`${incidentTimeline?.critical_count ?? 0} critical`}
                      />
                      <StatusBadge
                        ok={(incidentTimeline?.warning_count ?? 0) === 0}
                        label={`${incidentTimeline?.warning_count ?? 0} warning`}
                      />
                    </div>
                  </div>
                  <div className="divide-y divide-border">
                    {(incidentTimeline?.data ?? []).slice(0, 8).map((item) => (
                      <Link
                        key={`${item.key}:${item.occurred_at}:${item.entity_id ?? ''}`}
                        to={item.route}
                        className="grid gap-4 px-6 py-4 text-body hover:bg-canvas-sunk/50 md:grid-cols-[10rem_minmax(0,1fr)_8rem]"
                      >
                        <div>
                          <div className="font-medium text-ink">{item.title}</div>
                          <div className="mt-1 text-small capitalize text-ink-muted">
                            {item.category} · {item.source.replace('_', ' ')}
                          </div>
                        </div>
                        ...
                        <div className="flex items-start justify-end">
                          <span
                            className={`rounded-md border px-2 py-1 text-small font-medium ${item.severity === 'critical' ? 'border-danger/20 bg-danger/10 text-danger' : 'border-warn/20 bg-warn/10 text-warn-text'}`}
                          >
                            {item.severity}
                          </span>
                        </div>
                ...
                <aside className="rounded-md border border-border bg-canvas-raised">
                  <div className="border-b border-border px-6 py-4">
                    ...
                    <p className="text-small text-ink-muted">
                      {alertRules?.triggered_count ?? 0} triggered of {alertRules?.total ?? 0}
                    </p>
                  </div>
                  <div className="divide-y divide-border">
                    {(alertRules?.data ?? []).map((rule) => (
                      <Link
                        key={rule.key}
                        to={rule.route}
                        className="block px-6 py-4 hover:bg-canvas-sunk/50"
                      >
                        <div className="flex items-start justify-between gap-4">
                          <div>
                          ...
                          <span
                            className={`rounded-md border px-2 py-1 text-small font-medium ${rule.status === 'clear' ? 'border-accent-soft bg-accent-soft text-accent' : rule.severity === 'critical' ? 'border-danger/20 bg-danger/10 text-danger' : 'border-warn/20 bg-warn/10 text-warn-text'}`}
                          >
          ...
          {isSectionVisible('communications-governance', effectiveRole, viewMode) && (
            <PinnableSection sectionId="communications-governance" role={effectiveRole}>
              <ExpandableCard
                ...
                icon={CheckCircle2}
                status={
                  (outreachSummary?.blocked_count ?? 0) === 0 ? 'complete' : 'needs-attention'
                }
              ...
              >
                <div className="flex items-center justify-between gap-4">
                  <div>
                    ...
                    <div className="mt-1 text-small text-ink-muted">
                      Patient outreach requires channel consent and available contact details before
                      delivery is queued
                    </div>
                  </div>
                  <StatusBadge
                    ok={(outreachSummary?.blocked_count ?? 0) === 0}
                    label={`${outreachSummary?.blocked_count ?? 0} blocked`}
                  />
                </div>
                <div className="mt-3 grid gap-4 md:grid-cols-3">
                  {[
            ...
            <PinnableSection sectionId="operations-dashboard" role={effectiveRole}>
              ...
              >
                <div className="rounded-md border border-border bg-canvas-raised p-5">
                  ...
                  <div className="mt-3 font-serif text-3xl font-medium text-ink">
                    {analytics?.front_office.intake_needing_review ?? 0}
                  </div>
                ...
                <div className="rounded-md border border-border bg-canvas-raised p-5">
                  ...
                  <div className="mt-3 font-serif text-3xl font-medium text-ink">
                    {analytics?.billing.draft_cases ?? 0}
                  </div>
                  <div className="text-small text-ink-muted">
                    {analytics?.billing.denied_cases ?? 0} denied cases
                  </div>
                </div>
                <div className="rounded-md border border-border bg-canvas-raised p-5">
                  <div className="text-subhead font-semibold text-ink">Security Policy</div>
                  <div className="mt-3 font-serif text-3xl font-medium text-ink">
                    {sessionPolicy?.access_token_expire_minutes ?? '—'}m
                  </div>
                  <div className="text-small text-ink-muted">
                    {sessionPolicy?.mfa_required ? 'MFA required' : 'MFA staged for production'}
                  </div>
                </div>
                <div className="rounded-md border border-border bg-canvas-raised p-5">
                  <div className="text-subhead font-semibold text-ink">Assistant Governance</div>
                  <div className="mt-3 font-serif text-3xl font-medium text-ink">
                    {assistantEvents?.total ?? 0}
                  </div>
          ...
          {isSectionVisible('billing-work-queue', effectiveRole, viewMode) && (
            <PinnableSection sectionId="billing-work-queue" role={effectiveRole}>
              <ExpandableCard
                ...
                icon={ClipboardList}
                status={
                  (billingWorkQueue?.eligibility_needed_count ?? 0) === 0 &&
                  (billingWorkQueue?.missing_coding_count ?? 0) === 0
                    ? 'complete'
                    : 'needs-attention'
                }
                countComplete={
                  (billingWorkQueue?.ready_count ?? 0) + (billingWorkQueue?.submitted_count ?? 0)
                }
                countPending={
                  (billingWorkQueue?.denial_rework_count ?? 0) +
                  (billingWorkQueue?.remittance_pending_count ?? 0)
                }
              ...
              >
                <div className="flex items-center justify-between gap-4">
                  <div>
                    <div className="text-subhead font-semibold text-ink">
                      Billing Claim Governance
                    </div>
                    <div className="mt-1 text-small text-ink-muted">
                      Claim submission is gated by payer, coding, eligibility, and denial rework
                      status
                    </div>
                  </div>
                  <StatusBadge
                    ...
                    label={`${billingWorkQueue?.total ?? 0} cases`}
                  />
                </div>
                <div className="mt-3 grid gap-4 md:grid-cols-3">
                  {[
              ...
              >
                <div className="flex items-center justify-between border-b border-border px-6 py-4">
                  <div>
                    ...
                    <p className="text-small text-ink-muted">
                      {failedEvents.length} failed event{failedEvents.length === 1 ? '' : 's'} ready
                      for review
                    </p>
                  </div>
                  <ShieldCheck className="h-4 w-4 text-ink-faint" />
                  ...
                  <table className="w-full text-body">
                    ...
                    <tbody>
                      {(events?.data ?? []).map((event) => (
                        <tr
                          key={event.id}
                          className="border-b border-border-subtle hover:bg-canvas-sunk/50"
                        >
                          ...
                          <td className="px-6 py-4">
                            <StatusBadge ok={event.status === 'succeeded'} label={event.status} />
                          </td>
                      ...
                      {(events?.data ?? []).length === 0 && (
                        <tr>
                          <td
                            colSpan={6}
                            className="px-4 py-10 text-center text-body text-ink-faint"
                          >
