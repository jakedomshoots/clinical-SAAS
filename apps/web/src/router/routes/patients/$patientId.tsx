...
import type {
  ...
  PatientDocumentDownloadHandoff,
  PatientDocumentListResponse,
  PatientDocumentProcessResult,
  PatientEncounter,
  PatientEncounterListResponse,
  PatientLabResult,
  PatientLabResultListResponse,
  PatientMedication,
...
function PatientChartPage() {
  ...
  useDocumentTitle(
    patient
      ? `Chart: ${patient.last_name}, ${patient.first_name}`
      : 'Patient Chart'
  );
  ...
  const [documentReviewForms, setDocumentReviewForms] = useState<
    Record<string, DocumentReviewFormState>
  >({});
  ...
  const filteredDocuments = useMemo(() => {
    return documentRows.filter((doc) => {
      if (docSearchQuery) {
        ...
        if (!title.includes(query) && !src.includes(query)) return false;
      }
      if (docTypeFilter !== 'all') {
        const type = doc.document_type.toLowerCase();
        if (docTypeFilter === 'lab' && !type.includes('lab')) return false;
        if (docTypeFilter === 'referral' && !type.includes('referral')) return false;
        if (docTypeFilter === 'consult' && !type.includes('consult')) return false;
        if (docTypeFilter === 'other' && (type.includes('lab') || type.includes('referral') || type.includes('consult'))) return false;
      }
      return true;
    });
  ...
  const uniqueLabPanels = useMemo(() => {
    const panels = new Set(labRows.map((lab) => lab.panel));
    return Array.from(panels);
  }, [labRows]);

  useEffect(() => {
    if (uniqueLabPanels.length > 0 && !selectedLabPanel) {
      setSelectedLabPanel(uniqueLabPanels[0]);
    }
  }, [uniqueLabPanels, selectedLabPanel]);

  const labTrendData = useMemo(() => {
    if (!selectedLabPanel) return [];
    return labRows
      .filter((lab) => lab.panel === selectedLabPanel)
      .map((lab) => ({
        date: lab.collected_at ? new Date(lab.collected_at).toLocaleDateString() : '—',
        value: parseLabValue(lab.result),
        fullResult: lab.result,
        rawDate: lab.collected_at,
      }))
      .filter((item) => item.value !== null)
      .sort((a, b) => (a.rawDate || '').localeCompare(b.rawDate || ''));
  }, [labRows, selectedLabPanel]);
  ...
  const documentsNeedingReview = documentRows.filter(
  ...
  const updateMutation = useMutation({
    ...
    onError: (err) => {
      toast.error(err instanceof Error ? err.message : 'Failed to update patient demographics');
    },
  });
  ...
  const documentAccessMutation = useMutation({
    mutationFn: ({ documentId, reason }: { documentId: string; reason: string }) =>
      api.get<PatientDocumentAccess>(
        `${ROUTES.PATIENT_DOCUMENT_ACCESS(patientId, documentId)}?reason=${encodeURIComponent(reason)}`
      ),
      ...
      if (access.available && access.url) {
        if (access.url.startsWith('/api/')) {
          ...
          setDocumentAccessMessage(
            `${handoff.file_name}: ${handoff.message} Source ${handoff.source_uri_preview}. Access expires at ${handoff.expires_at ?? access.expires_at ?? 'the configured expiry time'}.`
          );
          return;
        }
        window.open(access.url, '_blank', 'noopener,noreferrer');
        setDocumentAccessMessage(
          `${access.viewer_mode === 'inline' ? 'Preview' : 'Download'} access expires at ${access.expires_at ?? 'the configured expiry time'}${access.content_type ? ` (${access.content_type})` : ''}.`
        );
      } else {
        setDocumentAccessMessage(
          access.reason ?? 'This document is not available for viewing yet.'
        );
      }
    },
    onError: (err) => {
      toast.error(err instanceof Error ? err.message : 'Failed to obtain document access');
    },
  });
  ...
  const processDocumentMutation = useMutation({
    mutationFn: (documentId: string) =>
      api.post<PatientDocumentProcessResult>(
        ROUTES.PATIENT_DOCUMENT_PROCESS(patientId, documentId),
        {}
      ),
    onSuccess: (result) => {
      queryClient.invalidateQueries({ queryKey: QUERY_KEYS.PATIENT_DOCUMENTS(patientId) });
  ...
  const formForDocumentReview = (document: PatientDocument): DocumentReviewFormState =>
    documentReviewForms[document.id] ?? {
  ...
  const updateDocumentReviewForm = (
    document: PatientDocument,
    patch: Partial<DocumentReviewFormState>
  ) => {
  ...
  const updateMedicationMutation = useMutation({
    ...
    onError: (err) => {
      toast.error(err instanceof Error ? err.message : 'Failed to update medication status');
    },
  });
  ...
  const updateCarePlanMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: QUERY_KEYS.PATIENT_CHECKOUT_HANDOFF(patientId) });
      toast.success('Care plan item updated');
    },
    onError: (err) => {
      toast.error(err instanceof Error ? err.message : 'Failed to update care plan item');
    },
  });
  ...
  const updateLabMutation = useMutation({
    ...
    onError: (err) => {
      toast.error(err instanceof Error ? err.message : 'Failed to update lab result status');
    },
  });
  ...
  const updateEncounterMutation = useMutation({
    ...
    onError: (err) => {
      toast.error(err instanceof Error ? err.message : 'Failed to update encounter status');
    },
  });
  ...
  const createEncounterMutation = useMutation({
    mutationFn: (templateId: string) => {
      ...
      return api.post<PatientEncounter>(ROUTES.PATIENT_ENCOUNTERS(patientId), {
        provider_id:
          staffRows.find((user) => user.role === 'provider')?.id ?? staffRows[0]?.id ?? null,
  ...
  const eligibilityMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: QUERY_KEYS.BILLING_CASES });
      toast.success('Eligibility check initiated');
    },
    onError: (err) => {
  ...
  const createHandoffTaskMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: QUERY_KEYS.PATIENT_CHECKOUT_HANDOFF(patientId) });
      queryClient.invalidateQueries({ queryKey: QUERY_KEYS.TASKS });
      toast.success('Handoff task generated');
    },
    onError: (err) => {
      toast.error(err instanceof Error ? err.message : 'Failed to generate handoff task');
    },
  });

  const completeCheckoutMutation = useMutation({
    mutationFn: (appointmentId: string) =>
      api.patch<Appointment>(`/schedule/appointments/${appointmentId}`, {
    ...
    onError: (mutationError) => {
      setCheckoutError(
        mutationError instanceof Error ? mutationError.message : 'Checkout could not be completed.'
      );
      toast.error(
        mutationError instanceof Error ? mutationError.message : 'Failed to complete checkout'
      );
    },
  });
  ...
  function saveEdit() {
    updateMutation.mutate({
      ...
      preferred_contact_channel:
        editForm.preferred_contact_channel === 'sms' ||
        editForm.preferred_contact_channel === 'email'
  ...
  if (isError) {
    return (
      <ErrorState
        ...
        detail={error instanceof Error ? error.message : 'The patient chart could not be loaded.'}
      />
    );
  }
  ...
  if (!patient) {
    return (
      <EmptyState
        ...
        detail="The selected chart is not available in the current frontend data set."
      />
    );
  }
  ...
  return (
    <div>
      ...
      <div className="mb-6 flex items-start justify-between">
        ...
        <Button onClick={() => setHandoffOpen(true)} icon={ShieldCheck}>
          Checkout Handoff
        </Button>
      ...
      <div className="mb-6 -mx-1 flex gap-0 overflow-x-auto border-b border-border px-1 scrollbar-none">
        {TABS.map(({ key, label }) => (
          <button
            ...
            className={`shrink-0 whitespace-nowrap px-4 py-2.5 text-small font-medium transition-colors ${
              activeTab === key
            ...
            {[
              {
                label: 'Visit state',
                value: 'Checkout prep',
                detail: 'Provider review pending',
                icon: Stethoscope,
                tone: 'text-accent',
              },
              {
                label: 'Documents',
                value: String(chartSummary?.counts.documents_total ?? documentRows.length),
                detail: `${chartSummary?.counts.documents_needing_review ?? documentsNeedingReview} needs review`,
                icon: FolderOpen,
                tone: 'text-warn',
              },
              {
                label: 'Open tasks',
                value: String(chartSummary?.counts.open_tasks ?? openTasks.length),
                detail: `${chartSummary?.counts.urgent_tasks ?? 0} urgent`,
                icon: ClipboardList,
                tone: 'text-danger',
              },
              {
                ...
                value: String(
                  (chartSummary?.counts.medications_needing_review ?? 0) +
                    (chartSummary?.counts.labs_needing_review ?? 0)
                ),
                detail: `${chartSummary?.counts.medications_needing_review ?? 0} meds, ${chartSummary?.counts.labs_needing_review ?? 0} labs`,
                icon: AlertTriangle,
                tone: 'text-warn',
              },
              {
                label: 'Care plan',
                value: String(carePlanItems.length),
                detail: `${chartSummary?.counts.care_plan_blockers ?? 0} blocked, ${chartSummary?.counts.unsigned_encounters ?? 0} unsigned`,
                icon: ShieldCheck,
                tone: 'text-ink-muted',
              },
              ...
              <div key={label} className="bg-canvas-raised border border-border rounded-md p-4">
                <div className="flex items-center justify-between">
                  ...
                  <div
                    className={`grid h-7 w-7 place-items-center rounded-md bg-canvas-sunk ${tone}`}
                  >
            ...
            <div className="border border-border bg-canvas-raised rounded-md">
              <div className="border-b border-border px-4 py-3">
                ...
                <p className="text-micro text-ink-muted mt-0.5">
                  What the care team needs before this patient leaves
                </p>
              </div>
              <div className="divide-y divide-border">
                {carePlanItems.map((item) => (
                  <div
                    key={item.id}
                    className="flex flex-col gap-2 px-4 py-3 md:flex-row md:items-center text-small"
                  >
                    <div className="w-full md:w-36 shrink-0 font-medium text-ink-secondary">
                      <select
                        value={item.assigned_to_id ?? ''}
                        onChange={(event) =>
                          updateCarePlanMutation.mutate({
                            itemId: item.id,
                            update: { assigned_to_id: event.target.value || null },
                          })
                        }
                      ...
                      {item.escalation && (
                        <div className="mt-0.5 text-micro text-danger">
                          {formatClinicalStatus(item.escalation)}
                        </div>
                      )}
                    </div>
                    <div className="flex-1 min-w-0 text-ink text-left">{item.item}</div>
                    <div className="w-28 shrink-0 text-ink-muted text-left md:text-right">
                      {item.due ?? 'No due date'}
                    </div>
                    <div className="w-28 shrink-0 font-medium text-ink-secondary text-left md:text-right">
              ...
              <div className="mt-3 space-y-3 text-small">
                {blockers.length > 0 ? (
                  blockers.map((blocker) => (
                    <div
                      key={blocker}
                      className="rounded-md border border-danger/20 bg-danger/10 p-3 text-danger"
                    >
                      {blocker}
                    </div>
                  ))
            ...
            <div className="max-w-md space-y-4">
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) =>
                    setEditForm({ ...editForm, preferred_contact_channel: e.target.value })
                  }
              ...
              <div className="flex flex-wrap gap-3">
                <label className="inline-flex items-center gap-2 text-small font-medium text-ink-secondary">
                  <input
                    type="checkbox"
                    checked={editForm.sms_consent === 'true'}
                    onChange={(e) =>
                      setEditForm({ ...editForm, sms_consent: e.target.checked ? 'true' : 'false' })
                    }
                ...
                <label className="inline-flex items-center gap-2 text-small font-medium text-ink-secondary">
                  <input
                    type="checkbox"
                    checked={editForm.email_consent === 'true'}
                    onChange={(e) =>
                      setEditForm({
                        ...editForm,
                        email_consent: e.target.checked ? 'true' : 'false',
                      })
                    }
                    className="h-4 w-4 rounded border-border text-accent"
                  />
              ...
              <div className="flex gap-2 pt-2">
                <Button onClick={saveEdit} disabled={updateMutation.isPending} icon={Check}>
                  {updateMutation.isPending ? 'Saving' : 'Save'}
                </Button>
                <Button variant="secondary" onClick={() => setEditing(false)} icon={X}>
                  Cancel
                </Button>
              </div>
            </div>
            ...
            <dl className="grid grid-cols-2 gap-4 max-w-lg text-small">
              {[
                ...
                { icon: Mail, label: 'Email', value: patient.email || '—' },
                {
                  ...
                  value: `${patient.sms_consent ? 'SMS' : 'No SMS'} / ${patient.email_consent ? 'Email' : 'No email'}`,
                },
                {
                  icon: ShieldCheck,
                  label: 'Preferred outreach',
                  value: patient.preferred_contact_channel || '—',
                },
                {
                  icon: MapPin,
                  label: 'Address',
            ...
            <div className="mt-6 border-t border-border pt-4">
              ...
              <div className="flex flex-wrap gap-2">
                {patient.allergies.map((a, i) => (
                  <span
                    key={i}
                    className="rounded-pill bg-warn/10 border border-warn/20 px-3 py-1 text-micro font-medium text-warn"
                  >
        ...
        <div className="border border-border bg-canvas-raised rounded-md">
            ...
            <div className="flex flex-wrap items-center justify-between gap-3">
              ...
              <div className="flex flex-wrap items-center gap-2">
                {(encounterTemplates?.data ?? []).map((template) => (
                  <button
                    key={template.id}
                    onClick={() => createEncounterMutation.mutate(template.id)}
          ...
          <div className="divide-y divide-border">
            {encounterRows.map((encounter) => (
              <div
                key={encounter.id}
                className="grid gap-3 px-4 py-3 md:grid-cols-[8rem_1fr_14rem]"
              >
                <div className="font-mono text-micro text-ink-muted">
                  {formatDateOnly(encounter.created_at)}
                </div>
                <div>
                  ...
                  <div className="mt-1 text-small text-ink-secondary">
                    {encounter.summary ?? 'No summary entered.'}
                  </div>
                  <div className="mt-1 text-micro text-ink-muted">
                    {encounter.provider_name ?? 'No provider assigned'}
                  </div>
                  {(encounter.assessment || encounter.plan) && (
                    <div className="mt-2 grid gap-2 text-micro text-ink-secondary md:grid-cols-2">
                      ...
                      {encounter.plan && (
                        <div>
                          <span className="font-medium text-ink">Plan:</span> {encounter.plan}
                        </div>
                      )}
                    </div>
                  )}
            ...
            {encounterRows.length === 0 && (
              <div className="px-4 py-8 text-center text-small text-ink-faint">
                No encounters have been added to this chart.
              </div>
            )}
          </div>
        </div>
        ...
        <div className="border border-border bg-canvas-raised rounded-md">
          <div className="border-b border-border px-4 py-3">
            ...
            <p className="mt-1 text-micro text-ink-muted">
              Faxed, scanned, and imported records from outside offices
            </p>
          </div>
          
          {/* Document Search & Filter bar */}
          <div className="flex flex-wrap items-center gap-3 bg-canvas-sunk/35 border-b border-border p-3">
            <div className="relative flex-1 min-w-[200px]">
              <Search className="absolute left-3 top-2 text-ink-faint h-4 w-4" />
              <input
                type="text"
                value={docSearchQuery}
                onChange={(e) => setDocSearchQuery(e.target.value)}
                placeholder="Search documents by title or source..."
                className="w-full bg-canvas border border-border rounded-md pl-9 pr-8 py-1 text-small text-ink placeholder:text-ink-faint focus:border-accent focus:outline-none"
              />
              {docSearchQuery && (
                <button
                  type="button"
                  onClick={() => setDocSearchQuery('')}
                  className="absolute right-2.5 top-2 text-ink-muted hover:text-ink cursor-pointer"
                >
                  <X className="h-4 w-4" />
                </button>
              )}
            ...
            <div className="flex items-center gap-1.5">
              ...
              >
                ...
                <option value="consult">Consult Notes</option>
          ...
          <div className="divide-y divide-border">
            ...
            {documentsLoading && (
              <div className="px-4 py-6 text-small text-ink-muted">
                Loading outside documents...
              </div>
            )}
            {documentsError && (
              <div className="px-4 py-6 text-small text-danger">
                Unable to load outside documents.
              </div>
            )}
            {!documentsLoading && !documentsError && filteredDocuments.length === 0 && (
              <div className="px-4 py-6 text-small text-ink-muted">
                No matching outside documents found.
              </div>
            )}
            {!documentsLoading &&
              !documentsError &&
              filteredDocuments.map((document) => {
                ...
                return (
                  <div
                    key={document.id}
                    className="grid gap-3 px-4 py-4 lg:grid-cols-[1fr_13rem_10rem_6rem]"
                  >
                    <div className="min-w-0">
                      ...
                      <div className="mt-2 grid gap-2 text-micro text-ink-muted md:grid-cols-2 xl:grid-cols-4">
                        <div className="rounded-md border border-border bg-canvas-sunk px-2 py-1">
                          <div className="font-medium text-ink-secondary">
                            {document.source_contact ?? 'Source contact not set'}
                          </div>
                          <div className="mt-0.5 flex flex-wrap gap-x-2 gap-y-0.5">
                            {document.source_phone && (
                              <span className="inline-flex items-center gap-1">
                        ...
                        <div className="rounded-md border border-border bg-canvas-sunk px-2 py-1">
                          ...
                          <div className="mt-0.5">
                            {document.source_reference ?? 'Not provided'}
                          </div>
                        </div>
                        <div className="rounded-md border border-border bg-canvas-sunk px-2 py-1">
                      ...
                      {document.review_note && (
                        <p className="mt-2 max-w-3xl rounded-md border border-accent-soft bg-accent-soft px-2 py-1.5 text-micro text-accent">
                          {document.review_note}{' '}
                          {document.reviewed_by ? `- ${document.reviewed_by}` : ''}{' '}
                          {document.reviewed_at ? `(${formatDateTime(document.reviewed_at)})` : ''}
                        </p>
                      )}
                      <div className="mt-2 flex flex-wrap gap-2 text-micro font-medium text-ink-muted">
                        ...
                        {document.classification && (
                          <Badge intent="warn">{document.classification.replace('_', ' ')}</Badge>
                        )}
                      </div>
                      <div className="mt-3 flex flex-wrap items-center gap-2 rounded-md border border-border bg-canvas p-2">
                        <select
                          value={reviewForm.status}
                          onChange={(event) =>
                            updateDocumentReviewForm(document, {
                              status: event.target.value as PatientDocument['status'],
                            })
                          }
                          className="bg-canvas border border-border rounded-sm px-2 py-1.5 text-micro text-ink focus:border-accent focus:ring-1 focus:ring-accent-soft focus:outline-none w-32"
                        >
                        ...
                        <input
                          ...
                          onChange={(event) =>
                            updateDocumentReviewForm(document, {
                              routed_to_role: event.target.value,
                            })
                          }
                          placeholder="Route role"
                          className="min-w-0 w-32 bg-canvas border border-border rounded-sm px-2 py-1.5 text-micro text-ink placeholder:text-ink-faint focus:border-accent focus:ring-1 focus:ring-accent-soft focus:outline-none"
                        />
                        <select
                          ...
                          className="bg-canvas border border-border rounded-sm px-2 py-1.5 text-micro text-ink focus:border-accent focus:ring-1 focus:ring-accent-soft focus:outline-none w-28"
                        >
                        ...
                        <input
                          ...
                          onChange={(event) =>
                            updateDocumentReviewForm(document, { reviewed_by: event.target.value })
                          }
                        ...
                        <input
                          ...
                          onChange={(event) =>
                            updateDocumentReviewForm(document, { review_note: event.target.value })
                          }
                          placeholder="Review note"
                          className="flex-1 min-w-[12rem] bg-canvas border border-border rounded-sm px-2 py-1.5 text-micro text-ink placeholder:text-ink-faint focus:border-accent focus:ring-1 focus:ring-accent-soft focus:outline-none"
                        />
                        <Button
                          variant="secondary"
                          size="sm"
                    ...
                    <div className="text-small text-ink-muted">
                      {document.file_url ? 'Available in chart' : 'Metadata only'}
                    </div>
          ...
          <div className="border-t border-border bg-canvas-sunk px-4 py-3">
            ...
            <div className="grid gap-2 md:grid-cols-2">
              {(accessHistory?.data ?? []).slice(0, 4).map((event) => (
                <div
                  key={event.id}
                  className="rounded-md border border-border bg-canvas-raised px-3 py-2 text-micro"
                >
                  <div className="font-medium text-ink-secondary">
                  ...
                  {typeof event.payload?.reason === 'string' && (
                    <div className="mt-1 text-ink-secondary">{event.payload.reason}</div>
                  )}
                </div>
              ))}
              {(accessHistory?.data ?? []).length === 0 && (
        ...
        <div className="fixed inset-0 z-50 bg-ink/20 backdrop-blur-sm p-4">
          <form
            onSubmit={(event) => {
              ...
              documentAccessMutation.mutate({
                documentId: documentAccessRequest.id,
          ...
          >
            <div className="flex items-center justify-between border-b border-border px-5 py-4">
              ...
              <button
                type="button"
                onClick={() => setDocumentAccessRequest(null)}
            ...
            <div className="flex justify-end gap-3 border-t border-border px-5 py-4">
              <Button variant="ghost" onClick={() => setDocumentAccessRequest(null)}>
                Cancel
              </Button>
              <Button
                disabled={
                  documentAccessMutation.isPending || documentAccessReason.trim().length < 3
                }
              >
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                ...
                {medicationRows.length === 0 && (
                  <tr>
                    <td colSpan={5} className="px-4 py-8 text-center text-small text-ink-faint">
                      No medications have been added to this chart.
                    </td>
                  </tr>
                )}
              </tbody>
          ...
          <div className="divide-y divide-border">
            {carePlanItems.map((item) => (
              ...
              >
                ...
                <div className="w-44 shrink-0 flex items-center justify-start md:justify-end gap-2 font-medium text-ink-secondary">
                  <span>{formatClinicalStatus(item.status)}</span>
            ...
            {carePlanItems.length === 0 && (
              <div className="px-4 py-8 text-center text-small text-ink-faint">
                No care-plan items have been added to this chart.
              </div>
            )}
          </div>
        </div>
      ...
      {activeTab === 'labs' && (
        <div className="border border-border bg-canvas-raised rounded-md space-y-4">
          <div className="border-b border-border px-4 py-3 flex flex-wrap items-center justify-between gap-3">
            ...
            {uniqueLabPanels.length > 0 && (
              <div className="flex items-center gap-2">
                <span className="text-micro font-medium text-ink-muted">Select Panel:</span>
                <select
                  value={selectedLabPanel}
                  onChange={(e) => setSelectedLabPanel(e.target.value)}
                  className="h-8 rounded-md border border-border bg-canvas px-2 text-small text-ink focus:border-accent focus:outline-none"
                >
                  {uniqueLabPanels.map((panel) => (
          ...
          {/* Recharts LineChart for Lab Trends */}
          {selectedLabPanel && labTrendData.length > 1 && (
            <div className="px-4 pb-2">
              <div className="border border-border bg-canvas/30 rounded-md p-3">
                <h3 className="text-micro font-semibold text-ink-muted mb-2">
                ...
                <div className="h-44 w-full">
                  <ResponsiveContainer width="100%" height="100%">
                    <LineChart data={labTrendData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                      ...
                      <YAxis stroke="var(--ink-muted)" fontSize={9} tickLine={false} domain={['auto', 'auto']} />
                      <Tooltip
                        contentStyle={{
            ...
            <tbody>
              {labRows.map((lab) => (
                <tr
                  ...
                  className="border-b border-border-subtle hover:bg-canvas-sunk/50 transition-colors duration-150"
                >
                  <td className="px-4 py-3 font-mono text-micro text-ink-muted">
                    {lab.collected_at ? formatDateOnly(lab.collected_at) : '—'}
                  </td>
                  ...
                  <td className="px-4 py-3">
                    <Badge
                      intent={
                        lab.flag === 'Critical' ? 'danger' : lab.flag === 'High' ? 'warn' : 'muted'
                      }
                    >
                      {lab.flag ?? 'Normal'}
                    </Badge>
                  ...
                  <td className="px-4 py-3 text-ink-muted">
                    ...
                    {lab.status === 'needs_review' && (
                      <Button
                        ...
                        className="ml-2"
                        onClick={() =>
                          updateLabMutation.mutate({ labId: lab.id, status: 'reviewed' })
                        }
                      >
                        Reviewed
                      </Button>
              ...
              {labRows.length === 0 && (
                <tr>
                  <td colSpan={5} className="px-4 py-8 text-center text-small text-ink-faint">
                    No lab results have been added to this chart.
                  </td>
                </tr>
              )}
            </tbody>
      ...
      {activeTab === 'billing' && (
        <div className="border border-border bg-canvas-raised rounded-md">
          <div className="flex items-center justify-between border-b border-border px-4 py-3">
            ...
            <Button variant="secondary" size="sm" onClick={() => eligibilityMutation.mutate()}>
              Check eligibility
            </Button>
          </div>
          <div className="divide-y divide-border">
              ...
              .map((item) => (
                <div key={item.id} className="grid gap-3 px-4 py-3 md:grid-cols-[1fr_10rem_10rem]">
                  <div>
                    ...
                    {item.notes && (
                      <div className="mt-1 text-micro text-ink-secondary">{item.notes}</div>
                    )}
                  </div>
            ...
            {(billingCases?.data ?? []).filter((item) => item.patient_id === patientId).length ===
              0 && (
              <div className="px-4 py-8 text-center text-small text-ink-faint">
                No billing cases for this patient.
              </div>
            )}
          </div>
              ...
              {(eligibilityHistory?.data ?? []).map((event) => (
                <div key={event.id} className="rounded-md bg-canvas-sunk px-3 py-2">
                  <div className="text-micro font-medium text-ink">
                    {String(event.payload.status ?? 'checked')}
                  </div>
                  <div className="mt-1 text-micro text-ink-muted">
                    {String(event.payload.payer ?? 'No payer')} ·{' '}
                    {new Date(event.created_at).toLocaleString()}
                  </div>
                </div>
              ))}
              {(eligibilityHistory?.data ?? []).length === 0 && (
                <div className="text-small text-ink-faint">No eligibility checks recorded yet.</div>
              )}
            </div>
          </div>
        </div>
      ...
      {activeTab === 'tasks' && (
        <div className="border border-border bg-canvas-raised rounded-md">
          ...
          <div className="divide-y divide-border">
            {openTasks.length === 0 && (
              <div className="px-4 py-6 text-small text-ink-muted">
                No open tasks are linked to this patient.
              </div>
            )}
            {openTasks.map((task) => (
              <div
                key={task.id}
                className="grid gap-3 px-4 py-3 md:grid-cols-[1fr_10rem_8rem_7rem]"
              >
      ...
      {handoffOpen && checkoutHandoff && (
        <CheckoutHandoffPanel
          ...
          onFileDocument={(documentId) =>
            updateDocumentMutation.mutate({ documentId, data: { status: 'filed' } })
          }
          onConfirmMedication={(medicationId) =>
            updateMedicationMutation.mutate({ medicationId, status: 'active' })
          }
          onReviewLab={(labId) => updateLabMutation.mutate({ labId, status: 'reviewed' })}
          onCompleteCarePlan={(itemId) =>
            updateCarePlanMutation.mutate({ itemId, update: { status: 'completed' } })
          }
          onSignEncounter={(encounterId) =>
            updateEncounterMutation.mutate({ encounterId, status: 'signed' })
          }
          onCreateTask={(sourceType, sourceId) =>
            createHandoffTaskMutation.mutate({ sourceType, sourceId })
          }
          onCompleteCheckout={() => {
            const appointment =
              checkoutHandoff.chart_summary.upcoming_appointments.find((item) =>
                ['checkout', 'provider_review', 'roomed', 'checked_in', 'in_progress'].includes(
                  item.status
                )
...
function CheckoutHandoffPanel({
  ...
  return (
    <div
      className="fixed inset-0 z-50 bg-ink/20 backdrop-blur-sm p-4"
      role="dialog"
      aria-modal="true"
    >
      <div className="ml-auto flex h-full max-w-3xl flex-col overflow-hidden rounded-md border border-border bg-canvas-raised shadow-lg">
        <div className="flex items-start justify-between gap-4 border-b border-border px-5 py-4">
          ...
          <button
            onClick={onClose}
            aria-label="Close checkout handoff"
            className="rounded-md p-1 text-ink-muted hover:bg-canvas-sunk hover:text-ink"
          >
            <X className="h-4 w-4" />
          </button>
        </div>
        ...
        <div className="min-h-0 flex-1 overflow-y-auto">
          <div
            className={`border-b px-4 py-3 text-small ${blocked ? 'border-danger/20 bg-danger/10 text-danger' : 'border-accent-soft bg-accent-soft text-accent'}`}
          >
            {blocked
              ? handoff.chart_summary.blockers.join('; ')
              : 'No chart blockers are currently reported.'}
          </div>

          <HandoffSection
            title="Documents Needing Review"
            rows={handoff.documents_needing_review.map((item) => ({
              id: item.id,
              title: item.title,
          ...
          <HandoffSection
            ...
            rows={handoff.medications_needing_review.map((item) => ({
              id: item.id,
              title: item.name,
          ...
          <HandoffSection
            title="Labs Needing Review"
            rows={handoff.labs_needing_review.map((item) => ({
              id: item.id,
          ...
          <HandoffSection
            title="Care Plan Open Items"
            rows={handoff.care_plan_open_items.map((item) => ({
              id: item.id,
              title: item.item,
          ...
          <HandoffSection
            ...
            rows={handoff.unsigned_encounters.map((item) => ({
              id: item.id,
        ...
        <div className="border-t border-border px-5 py-4">
          {checkoutError && (
            <div className="mb-2 rounded-md border border-danger/20 bg-danger/10 px-3 py-2 text-small text-danger">
              {checkoutError}
            </div>
          )}
          <div className="flex justify-end gap-3">
            ...
            <Button
              ...
              disabled={
                blocked || completing || handoff.chart_summary.upcoming_appointments.length === 0
              }
            >
