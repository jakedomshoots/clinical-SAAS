...
function PatientPortalPage() {
  ...
  const loginMutation = useMutation({
    mutationFn: () =>
      createApiClient(null).post<PortalLoginResponse>(ROUTES.PORTAL_AUTH_LOGIN, loginForm),
    onSuccess: (result) => {
  ...
  const intakeMutation = useMutation({
    ...
    onSuccess: (_, type) =>
      setLastAction(
        type === 'appointment_request' ? 'Appointment request sent.' : 'Intake update sent.'
      ),
  });
  ...
  const documentMutation = useMutation({
    mutationFn: async () => {
      ...
      const prepared = await api.post<PatientDocumentUploadPrepareResult>(
        ROUTES.PORTAL_PATIENT_DOCUMENT_UPLOAD,
        {
      ...
      <div className="mx-auto max-w-3xl px-5 py-8">
        <header className="mb-6 flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-small text-ink-muted mt-1">
              {patient ? `${patient.first_name} ${patient.last_name}` : 'Secure patient access'}
            </p>
          </div>
        </header>
        ...
        {!patient ? (
          <form
            ...
            className="rounded-md border border-border bg-canvas-raised p-4"
          >
            <div className="grid gap-3 md:grid-cols-3">
              ...
              <input
                type="date"
                value={loginForm.dob}
                onChange={(event) => setLoginForm({ ...loginForm, dob: event.target.value })}
                className="bg-canvas border border-border rounded-sm px-3 py-2 text-sm text-ink"
              />
              <input
                type="password"
                ...
                onChange={(event) =>
                  setLoginForm({ ...loginForm, access_code: event.target.value })
                }
                className="bg-canvas border border-border rounded-sm px-3 py-2 text-sm text-ink"
                placeholder="Access code"
              />
            </div>
            <button className="mt-3 rounded-md bg-accent text-accent-on px-4 py-2 text-sm font-medium hover:bg-accent-hover">
              Sign in
            </button>
          </form>
        ) : (
          <section className="space-y-4">
            <div className="rounded-md border border-border bg-canvas-raised p-4">
              <textarea
                ...
                onChange={(event) => setRequest({ ...request, reason: event.target.value })}
                className="min-h-24 w-full bg-canvas border border-border rounded-sm px-3 py-2 text-sm text-ink"
              />
              <div className="mt-3 flex flex-wrap gap-2">
                <button
                  onClick={() => intakeMutation.mutate('intake_form')}
            ...
            <div className="rounded-md border border-border bg-canvas-raised p-4">
              <div className="grid gap-3 md:grid-cols-2">
                <input
                  ...
                  onChange={(event) =>
                    setRequest({ ...request, appointment_date: event.target.value })
                  }
                  className="bg-canvas border border-border rounded-sm px-3 py-2 text-sm text-ink"
                />
                <input
                  type="time"
                  value={request.appointment_time}
                  onChange={(event) =>
                    setRequest({ ...request, appointment_time: event.target.value })
                  }
                  className="bg-canvas border border-border rounded-sm px-3 py-2 text-sm text-ink"
                />
              </div>
              <button
                onClick={() => intakeMutation.mutate('appointment_request')}
                className="mt-3 inline-flex items-center gap-2 rounded-md border border-border bg-canvas-raised text-ink-secondary px-3 py-2 text-sm font-medium hover:border-border-strong hover:bg-canvas-sunk"
              >
            ...
            <div className="rounded-md border border-border bg-canvas-raised p-4">
              <div className="grid gap-3 md:grid-cols-2">
                <input
                  ...
                  onChange={(event) =>
                    setRequest({ ...request, document_title: event.target.value })
                  }
                  className="bg-canvas border border-border rounded-sm px-3 py-2 text-sm text-ink"
                />
                <input
                  value={request.document_source}
                  onChange={(event) =>
                    setRequest({ ...request, document_source: event.target.value })
                  }
                  className="bg-canvas border border-border rounded-sm px-3 py-2 text-sm text-ink"
                />
              </div>
            ...
            {lastAction && (
              <div className="rounded-md border border-accent-soft bg-accent-soft px-3 py-2 text-sm font-medium text-accent">
                {lastAction}
              </div>
            )}
          </section>
