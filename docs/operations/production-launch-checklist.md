...
- `/api/operations/alert-rules` has no critical triggered rules, and `/api/operations/incident-timeline` has been reviewed for failed integrations, blocked logins, expired onboarding, backup/restore gaps, document access review signals, staff role/access warnings, and document-storage readiness.
