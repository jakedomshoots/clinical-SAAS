...
- Patients includes a Document Intake Workbench with status, role, and priority filters plus inline routing, reviewer, review-note, and filing actions.
...
- Use Document Review Queue to spot outside records that need matching, routing, or provider review, then use the Patients Document Intake Workbench to filter, route, review, and file incoming records.
