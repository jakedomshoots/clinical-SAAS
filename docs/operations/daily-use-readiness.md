...
- Integration Setup includes credential preflight for vendor env/draft fields, vendor owner/support profiles, adapter contract method readiness, placeholder-adapter blockers, connection-test status, sandbox workflow evidence, and explicit `local_sandbox` versus `production_vendor` readiness labels.
