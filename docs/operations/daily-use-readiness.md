...
- Integration Setup includes credential preflight for vendor env/draft fields, vendor owner/support profiles, per-integration cutover rehearsal evidence, vendor risk registers, vendor handoff packet export/archive evidence, adapter contract method readiness, placeholder-adapter blockers, connection-test status, sandbox workflow evidence, and explicit `local_sandbox` versus `production_vendor` readiness labels.
- Operations includes a live-use rehearsal board, Operator Health, document storage readiness, a go-live packet, credential dry-run binder, adapter implementation packet, integration cutover readiness packet, vendor credential request packet, incident timeline, local alert rules, browser QA evidence, role dry-run checklists, dry-run session evidence, staff training evidence, policy approval evidence, restore drill evidence, cutover runbook evidence, vendor handoff archive gates, launch workplan, and production rehearsal report that combine readiness, incidents, closeout, credential preflight, expiring document handoff/storage gaps, access review, backup/restore freshness, restore RTO/RPO evidence, launch evidence freshness, owner assignment, vendor lane assignment, rollback decisions, manager sign-off, workplan snapshots, credential binder snapshots, adapter implementation exports, integration cutover go/no-go exports, vendor request exports, CSV export, and audit-backed rehearsal/training/policy/restore/cutover/vendor handoff evidence.
...
- Use Credential Dry-Run Binder to export and save each integration's credential state, vendor owner, sandbox reference coverage, archive reference, blockers, and production-vendor readiness before requesting live credentials.
- Use Adapter Implementation Packet to review each placeholder adapter's required methods, mapped workflows, sandbox tests, credential prerequisites, docs, priority, and production-vendor evidence gap before assigning integration engineering work.
- Use Integration Cutover Readiness Packet to review one vendor lane at a time across adapter implementation, credential request readiness, handoff archive state, cutover rehearsal evidence, sandbox references, risks, blockers, next actions, go/hold/no-go status, and audit-backed lane owner assignment that feeds the Launch Workplan.
- Use Vendor Credential Request Packet to prepare vendor-specific credential outreach with required fields, missing values, owner/contact readiness, handoff archive context, sandbox reference coverage, checklist items, and a draft request body.
...
- Set `WEBHOOK_SHARED_SECRET` and configure vendors to send `X-Concierge-Webhook-Timestamp`, `X-Concierge-Webhook-Signature`, plus stable callback `event_id` values before enabling callbacks.
...
- Completed Adapter Implementation Packet review showing every live integration has production adapter contract methods implemented or assigned before vendor credential cutover.
